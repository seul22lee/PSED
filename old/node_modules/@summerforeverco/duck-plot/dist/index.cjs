...
const hasProperty = (prop) => Array.isArray(prop) ? prop.length > 0 : prop !== void 0;
function columnIsDefined(column2, config) {
  return Array.isArray(config[column2]) ? config[column2].filter((d) => d).length > 0 : config[column2] != null && config[column2] != void 0;
}
...
function getTransformType(type2, { x: x2, y: y2, series }) {
  if (isHorizontalX(type2)) {
    if (Array.isArray(x2) && x2.length > 1) {
      return hasProperty(series) ? "unPivotWithSeries" : "unPivot";
    }
  } else {
    if (Array.isArray(y2) && y2.length > 1) {
      return hasProperty(series) ? "unPivotWithSeries" : "unPivot";
    }
  }
  return "standard";
}
function getStandardTransformQuery(type2, { x: x2, y: y2, series, fy, fx, r, text: text2 }, tableName, into) {
  let select2 = [];
  if (hasProperty(x2)) select2.push(standardColName({ x: x2 }, "x"));
  if (hasProperty(fx)) select2.push(standardColName({ fx }, "fx"));
  if (hasProperty(y2)) select2.push(standardColName({ y: y2 }, "y"));
  if (hasProperty(series)) select2.push(maybeConcatCols(series, "series"));
  if (hasProperty(fy)) select2.push(maybeConcatCols(fy, "fy"));
  if (hasProperty(r)) select2.push(standardColName({ r }, "r"));
  if (hasProperty(text2)) select2.push(standardColName({ text: text2 }, "text"));
  return `CREATE TABLE ${into} as SELECT ${select2.join(
    ", "
  )} FROM ${tableName}`;
}
function getUnpivotQuery(type2, { x: x2, y: y2, fy, fx, r, text: text2 }, tableName, into) {
  var _a, _b;
  const createStatment = `CREATE TABLE ${into} as`;
  const selectStr = isHorizontalX(type2) ? `SELECT ${columnIsDefined("y", { y: y2 }) ? `"${y2}" as y, ` : ""}value AS x` : `SELECT ${columnIsDefined("fx", { fx }) ? `"${fx}" as fx, ` : ""}
        ${columnIsDefined("x", { x: x2 }) ? `"${x2}" as x, ` : ""}value AS y`;
  const keysStr = isHorizontalX(type2) ? (_a = quoteColumns(x2)) == null ? void 0 : _a.join(", ") : (_b = quoteColumns(y2)) == null ? void 0 : _b.join(", ");
  const fyStr = fy ? maybeConcatCols(fy, "fy,") : "";
  const rStr = r ? `${standardColName({ r }, "r")} ,` : "";
  const textStr = text2 ? `${standardColName({ text: text2 }, "text")} ,` : "";
  return `${createStatment} ${selectStr}, ${rStr}${textStr}${fyStr} key AS series${fx && !x2 ? ", key AS x" : ""} FROM ${tableName}
        UNPIVOT (value FOR key IN (${keysStr}));`;
}
function getUnpivotWithSeriesQuery(type2, { x: x2, y: y2, series, fy, fx, r, text: text2 }, tableName, into) {
  var _a, _b, _c;
  const xStatement = columnIsDefined("x", { x: x2 }) ? isHorizontalX(type2) ? `${(_a = quoteColumns(x2)) == null ? void 0 : _a.join(", ")}` : `"${x2}" as x` : "";
  const xShouldBeSeries = fx && !x2 && Array.isArray(y2) && y2.length > 1 && columnIsDefined("series", { series });
  const yStatement = isHorizontalX(type2) ? `"${y2}" as y` : (_b = quoteColumns(y2)) == null ? void 0 : _b.join(", ");
  const unPivotStatement = `FOR pivotCol IN (${(_c = quoteColumns(
    isHorizontalX(type2) ? x2 : y2
  )) == null ? void 0 : _c.join(", ")})`;
  const createStatement = `CREATE TABLE ${into} AS`;
  const selectClause = [
    columnIsDefined("x", { x: x2 }) ? "x" : xShouldBeSeries ? `concat_ws('-', pivotCol, series) AS x` : null,
    columnIsDefined("r", { r }) ? "r" : null,
    columnIsDefined("text", { text: text2 }) ? "text" : null,
    "y",
    `concat_ws('-', pivotCol, series) AS series`,
    (fy == null ? void 0 : fy.length) ? "fy" : null,
    (fx == null ? void 0 : fx.length) ? "fx" : null
  ].filter(Boolean).join(", ");
  const innerSelectClause = [
    xStatement,
    yStatement,
    maybeConcatCols(series, "series"),
    (fy == null ? void 0 : fy.length) ? maybeConcatCols(fy, "fy") : null,
    (fx == null ? void 0 : fx.length) ? maybeConcatCols(fx, "fx") : null,
    (r == null ? void 0 : r.length) ? maybeConcatCols(r, "r") : null,
    (text2 == null ? void 0 : text2.length) ? maybeConcatCols(text2, "text") : null
  ].filter(Boolean).join(", ");
  return `
    ...
    UNPIVOT (${isHorizontalX(type2) ? "x" : "y"} ${unPivotStatement});
  `;
}
function getTransformQuery(type2, config, tableName, intoTable, description) {
  const transformType = getTransformType(type2, omit(config, ["fy"]));
  function formatColumnString(cols) {
    if (!cols || !cols.length) return "";
    return Array.isArray(cols) ? cols.filter((d) => d).join(", ") : cols;
  }
  if (transformType === "unPivotWithSeries") {
    const transformColumns = isHorizontalX(type2) ? config.x : config.y;
    description.value += `The columns ${formatColumnString(
      transformColumns
    )} were unpivoted and then concatenated with ${config.series}, creating colors for each column-series.
`;
    return getUnpivotWithSeriesQuery(type2, config, tableName, intoTable);
...
`;
    return getUnpivotQuery(type2, config, tableName, intoTable);
  } else {
    return getStandardTransformQuery(type2, config, tableName, intoTable);
  }
}
function arrayIfy(value) {
  if (!value) return [];
  if (!Array.isArray(value)) return [value];
  return value.filter((d) => d) || [];
}
function getFinalQuery(instance, config, columns, tableName, aggregate, description) {
  const type2 = instance.mark().type;
  const percent = instance.config().percent;
  const y2 = arrayIfy(config.y);
  const x2 = arrayIfy(config.x);
  const agg = aggregate ?? "sum";
  let aggregateSelection;
  let groupBy = [];
  let labels = {};
  if (isHorizontalX(type2)) {
    if (x2 && x2.length > 0 && aggregate !== false) {
      aggregateSelection = ` ${agg}(x::FLOAT) as x`;
      labels.x = `${capitalize(agg)} of ${getLabel(x2)}`;
    }
    groupBy = columns.filter((d) => d !== "x");
  } else {
    if (y2 && y2.length > 0 && aggregate !== false) {
      aggregateSelection = ` ${agg}(y::FLOAT) as y`;
      labels.y = `${capitalize(agg)} of ${getLabel(y2)}`;
    }
    groupBy = columns.filter((d) => d !== "y");
  }
  const orderBy = getOrder(groupBy, type2, x2, y2);
  if (aggregate !== false) {
    description.value += `The ${isHorizontalX(type2) ? "x" : "y"} values were aggregated with a ${agg} aggregation, grouped by ${groupBy.join(
      `, `
    )}.`;
  }
  const subquery = aggregate !== false ? `
    SELECT ${[...groupBy, aggregateSelection].filter(Boolean).join(", ")}
    FROM ${tableName}
    ${groupBy.length ? `GROUP BY ${groupBy.join(", ")}` : ""}` : `SELECT *${percent ? `, ROW_NUMBER() OVER () AS original_order` : ""} FROM ${tableName}`;
  let aggregateColumn = "";
  if (isHorizontalX(type2) && x2 && x2.length > 0) {
    aggregateColumn = percent ? ` (x / (SUM(x) OVER (PARTITION BY ${groupBy.filter((d) => d !== "series").join(", ")}))) * 100 as x` : "x";
  } else if (y2 && y2.length > 0) {
    aggregateColumn = percent ? ` (y / (SUM(y) OVER (PARTITION BY ${groupBy.filter((d) => d !== "series").join(", ")}))) * 100 as y` : "y";
  }
  if (percent) {
    description.value += ` The ${isHorizontalX(type2) ? "x" : "y"} values were calculated as a percentage of the total for each group.`;
  }
  ...
  FROM aggregated
  ${percent && aggregate === false ? `ORDER BY original_order` : orderBy ? `ORDER BY ${orderBy}` : ""}
)`;
  return {
    queryString,
    labels
...
function getOrder(groupBy, type2, x2, y2) {
  let orderBy;
  if (isHorizontalX(type2) && x2.length > 1 || type2 !== "barX" && y2.length > 1) {
    const orderByArray = isHorizontalX(type2) && x2.length > 1 ? x2 : y2;
    const exclude = type2 === "barY" && groupBy.includes("fx") ? ["series", "x"] : ["series"];
    orderBy = [...groupBy].filter((d) => !exclude.includes(d)).join(", ");
    let caseStatements = orderByArray.map((item, index) => `WHEN series = '${item}' THEN ${index + 1}`).join("\n");
    orderBy += `, CASE 
    ${caseStatements}
...
function maybeConcatCols(cols, as) {
  if (!cols || !cols.length) return "";
  const colName = as ? ` as ${as}` : "";
  const colArr = Array.isArray(cols) ? cols : [cols];
  if (colArr.length > 1) {
    return `concat_ws('-', ${colArr.filter((d) => d).map((d) => `"${d}"`).join(", ")})${colName}`;
  }
  return `"${colArr[0]}"${colName}`;
}
function standardColName(obj, column2, colName) {
  const col = Array.isArray(obj[column2]) ? obj[column2][0] : obj[column2];
  return `"${col}" as ${column2}`;
}
const quoteColumns = (columns) => {
  if (!columns) return [];
  return !Array.isArray(columns) ? [`"${columns}"`] : columns.map((str) => `"${str}"`);
};
function toTitleCase(value) {
  ...
  return !result.includes(" ") ? result : result.toLowerCase().split(" ").map(capitalize).join(" ");
}
function capitalize(str) {
  if (typeof str === "boolean") return str;
  return str.charAt(0).toUpperCase() + str.slice(1);
}
function getLabel(columnSpec) {
  var _a;
  return Array.isArray(columnSpec) ? (_a = columnSpec == null ? void 0 : columnSpec.filter((d) => d).map(toTitleCase)) == null ? void 0 : _a.join(", ") : toTitleCase(columnSpec);
}
function isHorizontalX(type2) {
  return type2 === "barX" || type2 === "rectX";
}
const runQuery = async (db, sql) => {
  const conn = await db.connect();
  try {
    const arrow2 = await conn.query(sql);
    return arrow2.toArray();
  } catch (error) {
...
function blurfImage(radius2) {
  const blur = blurf(radius2);
  return (T, S, start2, stop, step) => {
    start2 <<= 2, stop <<= 2, step <<= 2;
    blur(T, S, start2 + 0, stop + 0, step);
    blur(T, S, start2 + 1, stop + 1, step);
    blur(T, S, start2 + 2, stop + 2, step);
    blur(T, S, start2 + 3, stop + 3, step);
  };
}
...
function count$1(values2, valueof2) {
  let count2 = 0;
  if (valueof2 === void 0) {
    for (let value of values2) {
      if (value != null && (value = +value) >= value) {
...
function group$1(values2, ...keys) {
  return nest(values2, identity$9, identity$9, keys);
}
function rollup(values2, reduce, ...keys) {
  return nest(values2, identity$9, reduce, keys);
}
...
function groupSort(values2, reduce, key) {
  return (reduce.length !== 2 ? sort$1(rollup(values2, reduce, key), ([ak, av], [bk, bv]) => ascending$2(av, bv) || ascending$2(ak, bk)) : sort$1(group$1(values2, key), ([ak, av], [bk, bv]) => reduce(av, bv) || ascending$2(ak, bk))).map(([key2]) => key2);
}
...
function thresholdSturges(values2) {
  return Math.max(1, Math.ceil(Math.log(count$1(values2)) / Math.LN2) + 1);
}
function max$2(values2, valueof2) {
  let max2;
  if (valueof2 === void 0) {
    for (const value of values2) {
...
function min$2(values2, valueof2) {
  let min2;
  if (valueof2 === void 0) {
    for (const value of values2) {
...
function greatest(values2, compare = ascending$2) {
  let max2;
  let defined2 = false;
  if (compare.length === 1) {
    let maxValue;
    for (const element of values2) {
      ...
      if (defined2 ? ascending$2(value, maxValue) > 0 : ascending$2(value, value) === 0) {
        max2 = element;
        maxValue = value;
        defined2 = true;
      }
...
function quantileSorted(values2, p, valueof2 = number$5) {
  if (!(n = values2.length) || isNaN(p = +p)) return;
...
function thresholdFreedmanDiaconis(values2, min2, max2) {
  const c2 = count$1(values2), d = quantile$1(values2, 0.75) - quantile$1(values2, 0.25);
  return c2 && d ? Math.ceil((max2 - min2) / (2 * d * Math.pow(c2, -1 / 3))) : 1;
}
function thresholdScott(values2, min2, max2) {
  const c2 = count$1(values2), d = deviation(values2);
...
function rank(values2, valueof2 = ascending$2) {
  if (typeof values2[Symbol.iterator] !== "function") throw new TypeError("values is not iterable");
  let V = Array.from(values2);
  const R = new Float64Array(V.length);
  if (valueof2.length !== 2) V = V.map(valueof2), valueof2 = ascending$2;
  const compareIndex = (i, j) => valueof2(V[i], V[j]);
  let k2, r;
  values2 = Uint32Array.from(V, (_, i) => i);
  values2.sort(valueof2 === ascending$2 ? (i, j) => ascendingDefined$1(V[i], V[j]) : compareDefined(compareIndex));
  values2.forEach((j, i) => {
...
function least(values2, compare = ascending$2) {
  let min2;
  let defined2 = false;
  if (compare.length === 1) {
    let minValue;
    for (const element of values2) {
      const value = compare(element);
      if (defined2 ? ascending$2(value, minValue) < 0 : ascending$2(value, value) === 0) {
        min2 = element;
        minValue = value;
        defined2 = true;
      }
...
function sum$1(values2, valueof2) {
  let sum2 = 0;
  if (valueof2 === void 0) {
    for (let value of values2) {
      if (value = +value) {
...
function center(scale2, offset2) {
  ...
  if (scale2.round()) offset2 = Math.round(offset2);
...
function axis(orient, scale2) {
  ...
  function axis2(context) {
    var values2 = tickValues == null ? scale2.ticks ? scale2.ticks.apply(scale2, tickArguments) : scale2.domain() : tickValues, format2 = tickFormat2 == null ? scale2.tickFormat ? scale2.tickFormat.apply(scale2, tickArguments) : identity$8 : tickFormat2, spacing = Math.max(tickSizeInner, 0) + tickPadding, range2 = scale2.range(), range0 = +range2[0] + offset2, range1 = +range2[range2.length - 1] + offset2, position2 = (scale2.bandwidth ? center : number$4)(scale2.copy(), offset2), selection2 = context.selection ? context.selection() : context, path = selection2.selectAll(".domain").data([null]), tick = selection2.selectAll(".tick").data(values2, scale2).order(), tickExit = tick.exit(), tickEnter = tick.enter().append("g").attr("class", "tick"), line2 = tick.select("line"), text2 = tick.select("text");
...
function color$1(format2) {
  ...
  return (m2 = reHex.exec(format2)) ? (l = m2[1].length, m2 = parseInt(m2[1], 16), l === 6 ? rgbn(m2) : l === 3 ? new Rgb(m2 >> 8 & 15 | m2 >> 4 & 240, m2 >> 4 & 15 | m2 & 240, (m2 & 15) << 4 | m2 & 15, 1) : l === 8 ? rgba(m2 >> 24 & 255, m2 >> 16 & 255, m2 >> 8 & 255, (m2 & 255) / 255) : l === 4 ? rgba(m2 >> 12 & 15 | m2 >> 8 & 240, m2 >> 8 & 15 | m2 >> 4 & 240, m2 >> 4 & 15 | m2 & 240, ((m2 & 15) << 4 | m2 & 15) / 255) : null) : (m2 = reRgbInteger.exec(format2)) ? new Rgb(m2[1], m2[2], m2[3], 1) : (m2 = reRgbPercent.exec(format2)) ? new Rgb(m2[1] * 255 / 100, m2[2] * 255 / 100, m2[3] * 255 / 100, 1) : (m2 = reRgbaInteger.exec(format2)) ? rgba(m2[1], m2[2], m2[3], m2[4]) : (m2 = reRgbaPercent.exec(format2)) ? rgba(m2[1] * 255 / 100, m2[2] * 255 / 100, m2[3] * 255 / 100, m2[4]) : (m2 = reHslPercent.exec(format2)) ? hsla(m2[1], m2[2] / 100, m2[3] / 100, 1) : (m2 = reHslaPercent.exec(format2)) ? hsla(m2[1], m2[2] / 100, m2[3] / 100, m2[4]) : named$1.hasOwnProperty(format2) ? rgbn(named$1[format2]) : format2 === "transparent" ? new Rgb(NaN, NaN, NaN, 0) : null;
}
function rgbn(n) {
...
function hue(a2, b) {
  var d = b - a2;
  return d ? linear$1(a2, d > 180 || d < -180 ? d - 360 * Math.round(d / 360) : d) : constant$5(isNaN(a2) ? b : a2);
}
function gamma(y2) {
  return (y2 = +y2) === 1 ? nogamma : function(a2, b) {
    return b - a2 ? exponential(a2, b, y2) : constant$5(isNaN(a2) ? b : a2);
  };
}
function nogamma(a2, b) {
  var d = b - a2;
  return d ? linear$1(a2, d) : constant$5(isNaN(a2) ? b : a2);
}
...
const { abs: abs$2, max: max$1, min: min$1 } = Math;
function number1(e) {
  return [+e[0], +e[1]];
}
function number2(e) {
...
function brush(dim) {
  var extent2 = defaultExtent, filter2 = defaultFilter, touchable = defaultTouchable, keys = true, listeners = dispatch("start", "brush", "end"), handleSize = 6, touchending;
  ...
  function redraw() {
    var group2 = select$1(this), selection2 = local(this).selection;
    if (selection2) {
  ...
  function started(event) {
    ...
    if (type2 === "overlay") {
      ...
      state.selection = selection2 = [[
        ...
        e0 = dim === Y ? E2 : max$1(pts[0][0], pts[1][0]),
    ...
    if (event.touches) {
      ...
      var view = select$1(event.view).on("mousemove.brush", moved, true).on("mouseup.brush", ended, true);
    ...
    function moved(event2) {
      for (const p of event2.changedTouches || [event2]) {
        for (const d of points)
          if (d.identifier === p.identifier) d.cur = pointof(p, that);
      }
      if (shifting && !lockX && !lockY && points.length === 1) {
        const point2 = points[0];
        if (abs$2(point2.cur[0] - point2[0]) > abs$2(point2.cur[1] - point2[1]))
          lockY = true;
        else
          lockX = true;
      }
    ...
    function move(event2) {
      ...
      switch (mode2) {
        case MODE_SPACE:
        case MODE_DRAG: {
          if (signX) dx = max$1(W - w0, min$1(E2 - e0, dx)), w1 = w0 + dx, e1 = e0 + dx;
          if (signY) dy = max$1(N - n0, min$1(S - s0, dy)), n1 = n0 + dy, s1 = s0 + dy;
          break;
        }
        case MODE_HANDLE: {
          if (points[1]) {
            if (signX) w1 = max$1(W, min$1(E2, points[0][0])), e1 = max$1(W, min$1(E2, points[1][0])), signX = 1;
            if (signY) n1 = max$1(N, min$1(S, points[0][1])), s1 = max$1(N, min$1(S, points[1][1])), signY = 1;
          } else {
            if (signX < 0) dx = max$1(W - w0, min$1(E2 - w0, dx)), w1 = w0 + dx, e1 = e0;
            else if (signX > 0) dx = max$1(W - e0, min$1(E2 - e0, dx)), w1 = w0, e1 = e0 + dx;
        ...
        case MODE_CENTER: {
          if (signX) w1 = max$1(W, min$1(E2, w0 - dx * signX)), e1 = max$1(W, min$1(E2, e0 + dx * signX));
  ...
  brush2.extent = function(_) {
    return arguments.length ? (extent2 = typeof _ === "function" ? _ : constant$4(number2(_)), brush2) : extent2;
  };
  brush2.filter = function(_) {
    return arguments.length ? (filter2 = typeof _ === "function" ? _ : constant$4(!!_), brush2) : filter2;
  };
  brush2.touchable = function(_) {
    return arguments.length ? (touchable = typeof _ === "function" ? _ : constant$4(!!_), brush2) : touchable;
  };
...
const pi$3 = Math.PI, tau$4 = 2 * pi$3, epsilon$4 = 1e-6, tauEpsilon = tau$4 - epsilon$4;
...
function contours() {
  ...
  function contours2(values2) {
    ...
    if (!Array.isArray(tz)) {
      const e = extent$1(values2, finite$2);
  ...
  function contour2(values2, value) {
    ...
    var polygons = [], holes = [];
    isorings(values2, v, function(ring) {
      smooth(ring, values2, v);
  ...
  function isorings(values2, value, callback) {
    ...
    x2 = y2 = -1;
    t12 = above(values2[0], value);
    cases[t12 << 1].forEach(stitch);
    while (++x2 < dx - 1) {
      t02 = t12, t12 = above(values2[x2 + 1], value);
      cases[t02 | t12 << 1].forEach(stitch);
    }
    cases[t12 << 0].forEach(stitch);
    while (++y2 < dy - 1) {
      x2 = -1;
      t12 = above(values2[y2 * dx + dx], value);
      t22 = above(values2[y2 * dx], value);
      cases[t12 << 1 | t22 << 2].forEach(stitch);
      while (++x2 < dx - 1) {
        t02 = t12, t12 = above(values2[y2 * dx + dx + x2 + 1], value);
        t32 = t22, t22 = above(values2[y2 * dx + x2 + 1], value);
  ...
  function smoothLinear(ring, values2, value) {
    ring.forEach(function(point2) {
      var x2 = point2[0], y2 = point2[1], xt = x2 | 0, yt = y2 | 0, v1 = valid(values2[yt * dx + xt]);
      if (x2 > 0 && x2 < dx && xt === x2) {
        point2[0] = smooth1(x2, valid(values2[yt * dx + xt - 1]), v1, value);
      }
      if (y2 > 0 && y2 < dy && yt === y2) {
        point2[1] = smooth1(y2, valid(values2[(yt - 1) * dx + xt]), v1, value);
      }
    });
  }
  contours2.contour = contour2;
  ...
  contours2.thresholds = function(_) {
    return arguments.length ? (threshold2 = typeof _ === "function" ? _ : Array.isArray(_) ? constant$3(slice$1.call(_)) : constant$3(_), contours2) : threshold2;
  };
  contours2.smooth = function(_) {
    return arguments.length ? (smooth = _ ? smoothLinear : noop$2, contours2) : smooth === smoothLinear;
  };
  return contours2;
}
function finite$2(x2) {
...
function contourDensity() {
  var x2 = defaultX, y2 = defaultY, weight = defaultWeight, dx = 960, dy = 500, r = 20, k2 = 2, o = r * 3, n = dx + o * 2 >> k2, m2 = dy + o * 2 >> k2, threshold2 = constant$3(20);
  function grid(data) {
    var values2 = new Float32Array(n * m2), pow2k = Math.pow(2, -k2), i = -1;
    for (const d of data) {
  ...
  function density2(data) {
    ...
    if (!Array.isArray(tz)) {
      tz = ticks(Number.MIN_VALUE, max$2(values2) / pow4k, tz);
    }
    return contours().size([n, m2]).thresholds(tz.map((d) => d * pow4k))(values2).map((c2, i) => (c2.value = +tz[i], transform(c2)));
  }
  density2.contours = function(data) {
    var values2 = grid(data), contours$1 = contours().size([n, m2]), pow4k = Math.pow(2, 2 * k2), contour2 = (value) => {
      value = +value;
      var c2 = transform(contours$1.contour(values2, value * pow4k));
      c2.value = value;
      return c2;
    };
    Object.defineProperty(contour2, "max", { get: () => max$2(values2) / pow4k });
    return contour2;
  };
  ...
  density2.x = function(_) {
    return arguments.length ? (x2 = typeof _ === "function" ? _ : constant$3(+_), density2) : x2;
  };
  density2.y = function(_) {
    return arguments.length ? (y2 = typeof _ === "function" ? _ : constant$3(+_), density2) : y2;
  };
  density2.weight = function(_) {
    return arguments.length ? (weight = typeof _ === "function" ? _ : constant$3(+_), density2) : weight;
  };
  ...
  density2.thresholds = function(_) {
    return arguments.length ? (threshold2 = typeof _ === "function" ? _ : Array.isArray(_) ? constant$3(slice$1.call(_)) : constant$3(_), density2) : threshold2;
  };
...
function centroidLinePoint(lambda, phi2) {
  lambda *= radians$1, phi2 *= radians$1;
  var cosPhi = cos$1(phi2), x2 = cosPhi * cos$1(lambda), y2 = cosPhi * sin$1(lambda), z = sin$1(phi2), w = atan2$1(sqrt$1((w = y0$4 * z - z0 * y2) * w + (w = z0 * x2 - x0$4 * z) * w + (w = x0$4 * y2 - y0$4 * x2) * w), x0$4 * x2 + y0$4 * y2 + z0 * z);
  W1 += w;
  X1$1 += w * (x0$4 + (x0$4 = x2));
  Y1$1 += w * (y0$4 + (y0$4 = y2));
  Z1$1 += w * (z0 + (z0 = z));
...
function longitude(point2) {
  return abs$1(point2[0]) <= pi$2 ? point2[0] : sign$1(point2[0]) * ((abs$1(point2[0]) + pi$2) % tau$2 - pi$2);
}
function polygonContains(polygon, point2) {
  var lambda = longitude(point2), phi2 = point2[1], sinPhi = sin$1(phi2), normal = [sin$1(lambda), -cos$1(lambda), 0], angle = 0, winding = 0;
  var sum2 = new Adder();
  ...
  for (var i = 0, n = polygon.length; i < n; ++i) {
    ...
    for (var j = 0; j < m2; ++j, lambda0 = lambda1, sinPhi0 = sinPhi1, cosPhi0 = cosPhi1, point0 = point1) {
      ...
      sum2.add(atan2$1(k2 * sign2 * sin$1(absDelta), cosPhi0 * cosPhi1 + k2 * cos$1(absDelta)));
...
function clipAntimeridianLine(stream) {
  ...
  return {
    ...
    point: function(lambda1, phi1) {
      var sign1 = lambda1 > 0 ? pi$2 : -pi$2, delta = abs$1(lambda1 - lambda0);
      if (abs$1(delta - pi$2) < epsilon$1) {
...
function clipCircle(radius2) {
  var cr = cos$1(radius2), delta = 2 * radians$1, smallRadius = cr > 0, notHemisphere = abs$1(cr) > epsilon$1;
  ...
  function intersect2(a2, b, two) {
    ...
    if (t22 < 0) return;
    var t = sqrt$1(t22), q = cartesianScale(u2, (-w - t) / uu);
    cartesianAddInPlace(q, A5);
    q = spherical(q);
    if (!two) return q;
    var lambda0 = a2[0], lambda1 = b[0], phi0 = a2[1], phi1 = b[1], z;
    if (lambda1 < lambda0) z = lambda0, lambda0 = lambda1, lambda1 = z;
    var delta2 = lambda1 - lambda0, polar = abs$1(delta2 - pi$2) < epsilon$1, meridian = polar || delta2 < epsilon$1;
    if (!polar && phi1 < phi0) z = phi0, phi0 = phi1, phi1 = z;
    if (meridian ? polar ? phi0 + phi1 > 0 ^ q[1] < (abs$1(q[0] - lambda0) < epsilon$1 ? phi0 : phi1) : phi0 <= q[1] && q[1] <= phi1 : delta2 > pi$2 ^ (lambda0 <= q[0] && q[0] <= lambda1)) {
...
function resample$1(project2, delta2) {
  function resampleLineTo(x02, y02, lambda0, a0, b0, c0, x12, y12, lambda1, a1, b1, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a2 = a0 + a1, b = b0 + b1, c2 = c0 + c1, m2 = sqrt$1(a2 * a2 + b * b + c2 * c2), phi2 = asin$1(c2 /= m2), lambda2 = abs$1(abs$1(c2) - 1) < epsilon$1 || abs$1(lambda0 - lambda1) < epsilon$1 ? (lambda0 + lambda1) / 2 : atan2$1(b, a2), p = project2(lambda2, phi2), x2 = p[0], y2 = p[1], dx2 = x2 - x02, dy2 = y2 - y02, dz = dy * dx2 - dx * dy2;
  ...
  return function(stream) {
    var lambda002, x002, y002, a00, b00, c00, lambda0, x02, y02, a0, b0, c0;
    var resampleStream = {
      point: point2,
      lineStart,
      lineEnd,
    ...
    function ringPoint(lambda, phi2) {
      linePoint(lambda002 = lambda, phi2), x002 = x02, y002 = y02, a00 = a0, b00 = b0, c00 = c0;
      resampleStream.point = linePoint;
    }
    function ringEnd() {
      resampleLineTo(x02, y02, lambda0, a0, b0, c0, x002, y002, lambda002, a00, b00, c00, maxDepth, stream);
...
function projectionMutator(projectAt) {
  var project2, k2 = 150, x2 = 480, y2 = 250, lambda = 0, phi2 = 0, deltaLambda = 0, deltaPhi = 0, deltaGamma = 0, rotate, alpha = 0, sx = 1, sy = 1, theta = null, preclip = clipAntimeridian, x02 = null, y02, x12, y12, postclip = identity$5, delta2 = 0.5, projectResample, projectTransform, projectRotateTransform, cache, cacheStream;
  ...
  projection2.center = function(_) {
    return arguments.length ? (lambda = _[0] % 360 * radians$1, phi2 = _[1] % 360 * radians$1, recenter()) : [lambda * degrees, phi2 * degrees];
  };
  ...
  function recenter() {
    var center2 = scaleTranslateRotate(k2, 0, 0, sx, sy, alpha).apply(null, project2(lambda, phi2)), transform = scaleTranslateRotate(k2, x2 - center2[0], y2 - center2[1], sx, sy, alpha);
...
function conicProjection$1(projectAt) {
  var phi0 = 0, phi1 = pi$2 / 3, m2 = projectionMutator(projectAt), p = m2(phi0, phi1);
  p.parallels = function(_) {
    return arguments.length ? m2(phi0 = _[0] * radians$1, phi1 = _[1] * radians$1) : [phi0 * degrees, phi1 * degrees];
  };
  return p;
}
function cylindricalEqualAreaRaw(phi0) {
  ...
  function forward(lambda, phi2) {
    return [lambda * cosPhi0, sin$1(phi2) / cosPhi0];
  }
  forward.invert = function(x2, y2) {
    return [x2 / cosPhi0, asin$1(y2 * cosPhi0)];
  };
  return forward;
}
...
function mercatorProjection(project2) {
  var m2 = projection$1(project2), center2 = m2.center, scale2 = m2.scale, translate = m2.translate, clipExtent = m2.clipExtent, x02 = null, y02, x12, y12;
  m2.scale = function(_) {
  ...
  function reclip() {
    var k2 = pi$2 * scale2(), t = m2(rotation(m2.rotate()).invert([0, 0]));
...
function conicConformalRaw(y02, y12) {
  ...
  project2.invert = function(x2, y2) {
    var fy = f - y2, r = sign$1(n) * sqrt$1(x2 * x2 + fy * fy), l = atan2$1(x2, abs$1(fy)) * sign$1(fy);
    if (fy * n < 0)
...
function shuffle$1(array2, random) {
...
function packEncloseRandom(circles, random) {
  var i = 0, n = (circles = shuffle$1(Array.from(circles), random)).length, B2 = [], p, e;
  while (i < n) {
    p = circles[i];
...
function pack() {
  ...
  function pack2(root2) {
    ...
    if (radius2) {
      root2.eachBefore(radiusLeaf(radius2)).eachAfter(packChildrenRandom(padding, 0.5, random)).eachBefore(translateChild(1));
    } else {
      root2.eachBefore(radiusLeaf(defaultRadius$1)).eachAfter(packChildrenRandom(constantZero, 1, random)).eachAfter(packChildrenRandom(padding, root2.r / Math.min(dx, dy), random)).eachBefore(translateChild(Math.min(dx, dy) / (2 * root2.r)));
    }
    return root2;
  }
  ...
  pack2.padding = function(x2) {
    return arguments.length ? (padding = typeof x2 === "function" ? x2 : constant$2(+x2), pack2) : padding;
  };
  return pack2;
}
...
function stratify() {
  ...
  function stratify2(data) {
    ...
    if (path != null) {
      const I = nodes.map((d2, i2) => normalize$2(path(d2, i2, data)));
...
function Tree() {
  ...
  function tree2(root2) {
    ...
    if (nodeSize) root2.eachBefore(sizeNode);
      ...
      var left = root2, right = root2, bottom2 = root2;
      root2.eachBefore(function(node) {
        if (node.x < left.x) left = node;
        if (node.x > right.x) right = node;
        if (node.depth > bottom2.depth) bottom2 = node;
      });
      var s2 = left === right ? 1 : separation(left, right) / 2, tx = s2 - left.x, kx2 = dx / (right.x + s2 + tx), ky2 = dy / (bottom2.depth || 1);
      root2.eachBefore(function(node) {
...
function squarifyRatio(ratio, parent, x02, y02, x12, y12) {
  ...
  while (i0 < n) {
    ...
    if (row.dice) treemapDice(row, x02, y02, x12, value ? y02 += dy * sumValue / value : y12);
    else treemapSlice(row, x02, y02, value ? x02 += dx * sumValue / value : x12, y12);
...
function transformer$1() {
  var domain = unit$1, range2 = unit$1, interpolate2 = interpolate$1, transform, untransform, unknown, clamp = identity$4, piecewise2, output, input;
  function rescale() {
...
function identity$3(domain) {
  ...
  scale2.domain = scale2.range = function(_) {
    return arguments.length ? (domain = Array.from(_, number$3), scale2) : domain.slice();
  };
  scale2.unknown = function(_) {
    return arguments.length ? (unknown = _, scale2) : unknown;
  };
...
function loggish(transform) {
  const scale2 = transform(transformLog$1, transformExp);
...
function log() {
  ...
  scale2.copy = () => copy$2(scale2, log()).base(scale2.base());
  initRange.apply(scale2, arguments);
  return scale2;
}
...
function symlogish(transform) {
  var c2 = 1, scale2 = transform(transformSymlog$1(c2), transformSymexp(c2));
...
function powish(transform) {
  var scale2 = transform(identity$4, identity$4), exponent2 = 1;
  function rescale() {
    return exponent2 === 1 ? transform(identity$4, identity$4) : exponent2 === 0.5 ? transform(transformSqrt$1, transformSquare) : transform(transformPow$1(exponent2), transformPow$1(1 / exponent2));
  }
...
function transformer() {
  var x02 = 0, x12 = 0.5, x2 = 1, s2 = 1, t02, t12, t22, k10, k21, interpolator = identity$4, transform, clamp = false, unknown;
  function scale2(x3) {
...
function cornerTangents(x02, y02, x12, y12, r1, rc, cw) {
  var x01 = x02 - x12, y01 = y02 - y12, lo = (cw ? rc : -rc) / sqrt(x01 * x01 + y01 * y01), ox2 = lo * y01, oy2 = -lo * x01, x11 = x02 + ox2, y11 = y02 + oy2, x10 = x12 + ox2, y10 = y12 + oy2, x002 = (x11 + x10) / 2, y002 = (y11 + y10) / 2, dx = x10 - x11, dy = y10 - y11, d2 = dx * dx + dy * dy, r = r1 - rc, D2 = x11 * y10 - x10 * y11, d = (dy < 0 ? -1 : 1) * sqrt(max(0, r * r * d2 - D2 * D2)), cx0 = (D2 * dy - dx * d) / d2, cy0 = (-D2 * dx - dy * d) / d2, cx1 = (D2 * dy + dx * d) / d2, cy1 = (-D2 * dx + dy * d) / d2, dx0 = cx0 - x002, dy0 = cy0 - y002, dx1 = cx1 - x002, dy1 = cy1 - y002;
...
function shapeLine(x$1, y$1) {
  var defined2 = constant$1(true), context = null, curve = curveLinear, output = null, path = withPath(line2);
  ...
  line2.defined = function(_) {
    return arguments.length ? (defined2 = typeof _ === "function" ? _ : constant$1(!!_), line2) : defined2;
  };
...
function d3pie() {
  var value = identity$2, sortValues = descending, sort2 = null, startAngle = constant$1(0), endAngle = constant$1(tau$1), padAngle = constant$1(0);
  function pie(data) {
    var i, n = (data = array(data)).length, j, k2, sum2 = 0, index = new Array(n), arcs = new Array(n), a0 = +startAngle.apply(this, arguments), da = Math.min(tau$1, Math.max(-tau$1, endAngle.apply(this, arguments) - a0)), a1, p = Math.min(Math.abs(da) / n, padAngle.apply(this, arguments)), pa = p * (da < 0 ? -1 : 1), v;
    for (i = 0; i < n; ++i) {
    ...
    else if (sort2 != null) index.sort(function(i2, j2) {
...
const curveCardinal = function custom3(tension) {
...
const curveCardinalClosed = function custom4(tension) {
  function cardinal(context) {
...
const curveCardinalOpen = function custom5(tension) {
  function cardinal(context) {
...
const curveCatmullRom = function custom6(alpha) {
...
const curveCatmullRomClosed = function custom7(alpha) {
  function catmullRom(context) {
...
const curveCatmullRomOpen = function custom8(alpha) {
  function catmullRom(context) {
...
function ascendingDefined(a2, b) {
  return +defined(b) - +defined(a2) || ascending$2(a2, b);
}
function descendingDefined(a2, b) {
  return +defined(b) - +defined(a2) || descending$1(a2, b);
}
function nonempty(x2) {
  return x2 != null && `${x2}` !== "";
}
function finite$1(x2) {
  return isFinite(x2) ? x2 : NaN;
}
...
function orderof(values2) {
  ...
  return descending$1(first2, last);
}
...
function formatTimeInterval(name, type2, anchor) {
  const format2 = type2 === "time" ? timeFormat : utcFormat;
  if (anchor == null) {
    return format2(
...
function inferTimeFormat(type2, dates, anchor) {
  const step = max$2(pairs(dates, (a2, b) => Math.abs(b - a2)));
...
function formatConditional(format1, format2, template2) {
  return (x2, i, X3) => {
    ...
    return i !== j && X3[j] !== void 0 && f2 === format2(X3[j], j) ? f1 : template2(f1, f2);
  };
}
...
function valueof(data, value, type2) {
  ...
  return valueType === "string" ? isArrowTable(data) ? maybeTypedArrowify(data.getChild(value), type2) : maybeTypedMap(data, field(value), type2) : valueType === "function" ? maybeTypedMap(data, value, type2) : valueType === "number" || value instanceof Date || valueType === "boolean" ? map$1(data, constant(value), type2) : typeof (value == null ? void 0 : value.transform) === "function" ? maybeTypedArrayify(value.transform(data), type2) : maybeTake(maybeTypedArrayify(value, type2), data == null ? void 0 : data[reindex]);
}
function maybeTake(values2, index) {
  return values2 != null && index ? take(values2, index) : values2;
}
function maybeTypedMap(data, f, type2) {
  return map$1(data, isNumberType(type2) ? (d, i) => coerceNumber(f(d, i)) : f, type2);
}
...
const number$1 = (x2) => x2 == null ? x2 : +x2;
const first = (x2) => x2 ? x2[0] : void 0;
const second = (x2) => x2 ? x2[1] : void 0;
const third = (x2) => x2 ? x2[2] : void 0;
...
function coerceNumbers(values2) {
  return isNumberArray(values2) ? values2 : map$1(values2, coerceNumber, Float64Array);
}
function coerceNumber(x2) {
  return x2 == null ? NaN : Number(x2);
}
function coerceDates(values2) {
  return map$1(values2, coerceDate);
}
...
function maybeColorChannel(value, defaultValue) {
  ...
  return value === null ? [void 0, "none"] : isColor$1(value) ? [void 0, value] : [value, void 0];
}
...
function take(values2, index) {
  return isArray(values2) ? map$1(index, (i) => values2[i], values2.constructor) : map$1(index, (i) => values2.at(i));
}
function taker(f) {
...
function mid(x12, x2) {
  return {
    transform(data) {
      ...
      return isTemporal(X12) || isTemporal(X22) ? map$1(X12, (_, i) => new Date((+X12[i] + +X22[i]) / 2)) : map$1(X12, (_, i) => (+X12[i] + +X22[i]) / 2, Float64Array);
    },
    label: x12.label
  };
}
...
function reverse({ sort: sort2, ...options } = {}) {
  return {
...
function shuffle({ seed, sort: sort2, ...options } = {}) {
  return {
...
function groupX(outputs = { y: "count" }, options = {}) {
  const { x: x2 = identity$1 } = options;
  if (x2 == null) throw new Error("missing channel: x");
  return groupn(x2, null, outputs, options);
}
function groupY(outputs = { x: "count" }, options = {}) {
  const { y: y2 = identity$1 } = options;
  if (y2 == null) throw new Error("missing channel: y");
...
function groupn(x2, y2, {
  ...
  sort2 = sort2 == null ? void 0 : maybeGroupOutput("sort", sort2, inputs);
  ...
  return {
    ...
    ...basic(options, (data, facets, plotOptions) => {
      const X3 = maybeApplyInterval(valueof(data, x2), plotOptions == null ? void 0 : plotOptions.x);
      const Y3 = maybeApplyInterval(valueof(data, y2), plotOptions == null ? void 0 : plotOptions.y);
        ...
        for (const [f, I] of maybeGroup(facet, G)) {
          for (const [y3, gg] of maybeGroup(I, Y3)) {
            for (const [x3, g] of maybeGroup(gg, X3)) {
              ...
              if (Y3) extent2.y = y3;
              if (G) extent2.z = f;
              if (filter2 && !filter2.reduce(g, extent2)) continue;
              groupFacet.push(i++);
              groupData.push(reduceData.reduceIndex(g, data, extent2));
              if (X3) GX2.push(x3);
              if (Y3) GY2.push(y3);
              if (Z) GZ2.push(G === Z ? f : Z[g[0]]);
              if (F) GF2.push(G === F ? f : F[g[0]]);
              if (S) GS2.push(G === S ? f : S[g[0]]);
              for (const o of outputs) o.reduce(g, extent2);
              if (sort2) sort2.reduce(g, extent2);
            }
          }
        }
...
function maybeOutputs(outputs, inputs, asOutput = maybeOutput) {
  ...
  if (inputs.href != null && outputs.href === void 0) entries.push(["href", reduceFirst$1]);
...
function maybeReduce$1(reduce, value, fallback = invalidReduce) {
  ...
  switch (`${reduce}`.toLowerCase()) {
    ...
    case "min":
      return reduceAccessor$1(min$2);
    case "min-index":
      return reduceAccessor$1(minIndex);
    case "max":
      return reduceAccessor$1(max$2);
    case "max-index":
...
const reduceTitle = {
  reduceIndex(I, X3) {
    ...
    if (top.length < groups.length) {
      ...
      top[n - 1] = [`… ${bottom2.length.toLocaleString("en-US")} more`, sum$1(bottom2, second)];
    }
...
function reduceProportion(value, scope) {
  return value == null ? { scope, label: "Frequency", reduceIndex: (I, V, basis2 = 1) => I.length / basis2 } : { scope, reduceIndex: (I, V, basis2 = 1) => sum$1(I, (i) => V[i]) / basis2 };
}
const reduceX$1 = {
  reduceIndex(I, X3, { x: x2 }) {
    return x2;
  }
};
const reduceY$1 = {
  reduceIndex(I, X3, { y: y2 }) {
    return y2;
  }
};
const reduceZ = {
...
function createChannel(data, { scale: scale2, type: type2, value, filter: filter2, hint, label = labelof(value) }, name) {
...
function valueObject(channels, scales) {
  const values2 = Object.fromEntries(
    Object.entries(channels).map(([name, { scale: scaleName, value }]) => {
      ...
      return [name, scale2 == null ? value : map$1(value, scale2)];
    })
  );
...
function inferChannelScale(name, channel) {
  ...
  if (scale2 === true || scale2 === "auto") {
    switch (name) {
      ...
      case "stroke":
      case "color":
        channel.scale = scale2 !== true && isEvery(value, isColor$1) ? null : "color";
...
function channelDomain(data, facets, channels, facetChannels, options) {
  ...
  for (const x2 in options) {
    ...
    if (y2 == null) {
      ...
      const reducer2 = maybeReduce$1(reduce === true ? "max" : reduce, YV);
      X3.domain = () => {
        let domain = rollups(
          range(XV),
...
function difference(channels, k1, k2) {
  ...
  return map$1(X22, (x2, i) => Math.abs(x2 - X12[i]), Float64Array);
}
...
function createScaleQ(key, scale2, channels, {
  ...
  interpolate: interpolate2 = registry.get(key) === color ? scheme2 == null && range2 !== void 0 ? interpolateRgb : quantitativeScheme(scheme2 !== void 0 ? scheme2 : type2 === "cyclical" ? "rainbow" : "turbo") : round2 ? interpolateRound : interpolateNumber,
  reverse: reverse2
}) {
  interval2 = maybeRangeInterval(interval2, type2);
  ...
  if (range2 !== void 0) {
    ...
    const m2 = (range2 = arrayify(range2)).length;
    if (n !== m2) {
  ...
  if (reverse2) domain = reverse$1(domain);
  scale2.domain(domain).unknown(unknown);
  if (nice2) scale2.nice(maybeNice(nice2, type2)), domain = scale2.domain();
  if (range2 !== void 0) scale2.range(range2);
  if (clamp) scale2.clamp(clamp);
  return { type: type2, domain, range: range2, scale: scale2, interpolate: interpolate2, interval: interval2 };
}
...
function createScaleThreshold(key, channels, {
  ...
  if (reverse2) range2 = reverse$1(range2);
  return {
    type: "threshold",
    scale: threshold(sign2 < 0 ? reverse$1(domain) : domain, range2 === void 0 ? [] : range2).unknown(unknown),
    domain,
    range: range2
  };
}
function isOrdered(domain, sign2) {
  for (let i = 1, n = domain.length, d = domain[0]; i < n; ++i) {
    const s2 = descending$1(d, d = domain[i]);
    if (s2 !== 0 && s2 !== sign2) return false;
  }
  return true;
}
function createScaleIdentity(key) {
  return { type: "identity", scale: hasNumericRange(registry.get(key)) ? identity$3() : (d) => d };
}
function inferDomain$1(channels, f = finite$1) {
  return channels.length ? [
    min$2(channels, ({ value }) => value === void 0 ? value : min$2(value, f)),
    max$2(channels, ({ value }) => value === void 0 ? value : max$2(value, f))
  ] : [0, 1];
}
...
function inferZeroDomain(channels) {
  return [0, channels.length ? max$2(channels, ({ value }) => value === void 0 ? value : max$2(value, finite$1)) : 1];
}
...
function createScaleD(key, scale2, transform, channels, {
  ...
  if (descending$1(min2, max2) < 0) [min2, max2] = [max2, min2], reverse2 = !reverse2;
  ...
  return { type: type2, domain: [min2, max2], pivot, interpolate: interpolate2, scale: scale2 };
}
...
function createScaleO(key, scale2, channels, { type: type2, interval: interval2, domain, range: range2, reverse: reverse2, hint }) {
  interval2 = maybeRangeInterval(interval2, type2);
  ...
  return { type: type2, domain, range: range2, scale: scale2, hint, interval: interval2 };
}
function createScaleOrdinal(key, channels, { type: type2, interval: interval2, domain, range: range2, scheme: scheme2, unknown, ...options }) {
  interval2 = maybeRangeInterval(interval2, type2);
  if (domain === void 0) domain = inferDomain(channels, interval2, key);
  let hint;
  if (registry.get(key) === symbol) {
    hint = inferSymbolHint(channels);
    range2 = range2 === void 0 ? inferSymbolRange(hint) : map$1(range2, maybeSymbol);
  } else if (registry.get(key) === color) {
    if (range2 === void 0 && (type2 === "ordinal" || type2 === ordinalImplicit)) {
      range2 = maybeBooleanRange(domain, scheme2);
      if (range2 !== void 0) scheme2 = void 0;
    }
    if (scheme2 === void 0 && range2 === void 0) {
      scheme2 = type2 === "ordinal" ? "turbo" : "observable10";
    }
    if (scheme2 !== void 0) {
      if (range2 !== void 0) {
  ...
  return createScaleO(key, ordinal().unknown(unknown), channels, { ...options, type: type2, domain, range: range2, hint });
}
...
function inferDomain(channels, interval2, key) {
  ...
  if (values2.size > 1e4 && registry.get(key) === position$1) {
...
function createScales(channelsByScale, {
  ...
  for (const [key, channels] of channelsByScale) {
    ...
    const scale2 = createScale(key, channels, {
      round: registry.get(key) === position$1 ? round2 : void 0,
      // only for position
      nice: nice2,
      clamp,
      zero: zero2,
...
function createScaleFunctions(descriptors) {
  ...
  for (const [key, descriptor] of Object.entries(descriptors)) {
    const { scale: scale2, type: type2, interval: interval2, label } = descriptor;
    scales[key] = exposeScale(descriptor);
    scaleFunctions[key] = scale2;
    scale2.type = type2;
    if (interval2 != null) scale2.interval = interval2;
...
function autoScaleRangeY(scale2, dimensions) {
  if (scale2.range === void 0) {
    ...
    scale2.range = [Math.max(top, bottom2), top];
    if (!isOrdinalScale(scale2)) scale2.range = piecewiseRange(scale2);
...
function inferScaleType(key, channels, { type: type2, domain, range: range2, scheme: scheme2, pivot, projection: projection2 }) {
...
function coerceSymbols(values2) {
  return map$1(values2, maybeSymbol);
}
...
function exposeScale({ scale: scale2, type: type2, domain, range: range2, interpolate: interpolate2, interval: interval2, transform, percent, pivot }) {
...
function createFacets(channelsByScale, options) {
  ...
  return fxDomain && fyDomain ? cross$1(fxDomain, fyDomain).map(([x2, y2], i) => ({ x: x2, y: y2, i })) : fxDomain ? fxDomain.map((x2, i) => ({ x: x2, i })) : fyDomain ? fyDomain.map((y2, i) => ({ y: y2, i })) : void 0;
}
...
function facetAnchorTopEmpty(facets, { y: Y3 }, { x: x2, y: y2, empty: empty2 }) {
  if (empty2) return false;
  if (!Y3) return;
  const i = facetIndexOf(Y3, y2);
  if (i > 0) return facetEmpty(facets, x2, Y3[i - 1]);
}
function facetAnchorBottomEmpty(facets, { y: Y3 }, { x: x2, y: y2, empty: empty2 }) {
  if (empty2) return false;
  if (!Y3) return;
  const i = facetIndexOf(Y3, y2);
  if (i < Y3.length - 1) return facetEmpty(facets, x2, Y3[i + 1]);
}
function facetAnchorLeftEmpty(facets, { x: X3 }, { x: x2, y: y2, empty: empty2 }) {
  if (empty2) return false;
  if (!X3) return;
  const i = facetIndexOf(X3, x2);
  if (i > 0) return facetEmpty(facets, X3[i - 1], y2);
}
function facetAnchorRightEmpty(facets, { x: X3 }, { x: x2, y: y2, empty: empty2 }) {
  if (empty2) return false;
  if (!X3) return;
  const i = facetIndexOf(X3, x2);
...
function conicProjection(createProjection2, kx2, ky2) {
  const { type: type2, aspectRatio } = scaleProjection$1(createProjection2, kx2, ky2);
  return {
    type: (options) => {
...
function projectionAspectRatio(projection2) {
  ...
  if (isObject(projection2)) {
    ...
    if (domain != null && projection2 != null) {
      const type2 = typeof projection2 === "string" ? namedProjection(projection2).type : projection2;
      const [[x02, y02], [x12, y12]] = geoPath(type2({ ...options, width: 100, height: 100 })).bounds(domain);
...
function applyPosition(channels, scales, { projection: projection2 }) {
  ...
  let position2 = {};
  if (x2) position2.x = x2;
  if (y2) position2.y = y2;
  position2 = valueObject(position2, scales);
  if (projection2 && (x2 == null ? void 0 : x2.scale) === "x" && (y2 == null ? void 0 : y2.scale) === "y") project("x", "y", position2, projection2);
  if (x2) position2.x = coerceNumbers(position2.x);
  if (y2) position2.y = coerceNumbers(position2.y);
  return position2;
}
...
const monthFormat = memoize1((locale2, month) => {
  return new Intl.DateTimeFormat(locale2, { timeZone: "UTC", ...month && { month } });
});
const weekdayFormat = memoize1((locale2, weekday) => {
...
function formatMonth(locale2 = "en-US", format2 = "short") {
  const fmt = monthFormat(locale2, format2);
  return (i) => i != null && !isNaN(i = +new Date(Date.UTC(2e3, +i))) ? fmt.format(i) : void 0;
}
function formatWeekday(locale2 = "en-US", format2 = "short") {
  const fmt = weekdayFormat(locale2, format2);
...
function formatAuto(locale2 = "en-US") {
  const number3 = formatNumber(locale2);
...
function memoizeClip(clip2) {
  ...
  return (context, dimensions) => {
    ...
    if (!url) {
      const id2 = getClipId();
      select$1(context.ownerSVGElement).append("clipPath").attr("id", id2).call(clip2, context, dimensions);
...
function aspectRatioLength(k2, scale2) {
  if (!scale2) throw new Error(`aspectRatio requires ${k2} scale`);
  const { type: type2, domain } = scale2;
  let transform;
  switch (type2) {
    case "linear":
    case "utc":
    case "time":
      ...
      break;
    case "pow": {
      const exponent2 = scale2.scale.exponent();
...
function pointerK(kx2, ky2, { x: x2, y: y2, px, py, maxRadius = 40, channels, render: render2, ...options } = {}) {
  maxRadius = +maxRadius;
  ...
  return {
    ...
    render: composeRender(function(index, scales, values2, dimensions, context, next) {
      ...
      function pointerdown(event) {
        ...
        else state.sticky = true, render3(i);
        event.stopImmediatePropagation();
      }
...
function anchorX$1({ x1: X12, x2: X22, x: X3 = X12 }, cx) {
  return X12 && X22 ? (i) => (X12[i] + X22[i]) / 2 : X3 ? (i) => X3[i] : () => cx;
}
function anchorY$1({ y1: Y12, y2: Y22, y: Y3 = Y12 }, cy) {
  return Y12 && Y22 ? (i) => (Y12[i] + Y22[i]) / 2 : Y3 ? (i) => Y3[i] : () => cy;
}
function inferFontVariant$2(scale2) {
...
:where(.${className}-ramp text) {
  ...
  let x2;
  const applyRange = round2 ? (x3, range3) => x3.rangeRound(range3) : (x3, range3) => x3.range(range3);
  const { type: type2, domain, range: range2, interpolate: interpolate2, scale: scale2, pivot } = color2;
  if (interpolate2) {
    ...
    svg.append("g").attr("fill-opacity", opacity2).selectAll().data(range2).enter().append("rect").attr("x", (d, i) => x2(i - 1)).attr("y", marginTop).attr("width", (d, i) => x2(i) - x2(i - 1)).attr("height", height - marginTop - marginBottom).attr("fill", (d) => d);
    ticks2 = map$1(thresholds, (_, i) => i);
    tickFormat2 = (i) => thresholdFormat(thresholds[i], i);
  } else {
    x2 = applyRange(band().domain(domain), [marginLeft, width - marginRight]);
    svg.append("g").attr("fill-opacity", opacity2).selectAll().data(domain).enter().append("rect").attr("x", x2).attr("y", marginTop).attr("width", Math.max(0, x2.bandwidth() - 1)).attr("height", height - marginTop - marginBottom).attr("fill", scale2);
    tickAdjust = () => {
    };
  }
...
function getGroupedOrientation(path, Z) {
  ...
  const D2 = path.data().filter((I) => I.length > 1);
  const n = D2.length;
...
function maybeIntervalK(k2, maybeInsetK, options, trivial) {
  ...
  function transform(data) {
    ...
    return V1 = map$1(valueof(D1 = data, value), (v3) => interval2.floor(v3));
  }
  return maybeInsetK({
    ...options,
    [k2]: void 0,
...
function maybeIntervalMidK(k2, maybeInsetK, options) {
  ...
  return maybeInsetK({
    ...
    [k2]: {
      label: labelof(v),
      transform: (data) => {
        const V1 = map$1(valueof(data, value), (v2) => interval2.floor(v2));
...
class RuleX extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    return create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, { x: X3 && x2 }, offset, 0).call(
      (g) => g.selectAll().data(index).enter().append("line").call(applyDirectStyles, this).attr("x1", X3 ? (i) => X3[i] : (marginLeft + width - marginRight) / 2).attr("x2", X3 ? (i) => X3[i] : (marginLeft + width - marginRight) / 2).attr("y1", Y12 && !isCollapsed(y2) ? (i) => Y12[i] + insetTop : marginTop + insetTop).attr(
        "y2",
...
class RuleY extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    return create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, { y: Y3 && y2 }, 0, offset).call(
...
class Text extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    const { rotate } = this;
    const [cx, cy] = applyFrameAnchor(this, dimensions);
    return create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyIndirectTextStyles, this, T, dimensions).call(applyTransform, this, { x: X3 && x2, y: Y3 && y2 }).call(
  ...
  selection2.each(function(i) {
    ...
    if (n > 1) {
      ...
      for (let i2 = 0; i2 < n; ++i2) {
        ...
        if (i2 === m2 - 1) tspan.setAttribute("y", `${(y2 + i2) * lineHeight}em`);
        else tspan.setAttribute("dy", `${m2 * lineHeight}em`);
...
function clipper({ monospace, lineWidth, textOverflow }) {
  ...
  switch (textOverflow) {
    ...
    case "ellipsis-start":
      return (text2) => clipStart(text2, maxWidth, widthof, ellipsis$1);
    case "ellipsis-middle":
      return (text2) => clipMiddle(text2, maxWidth, widthof, ellipsis$1);
    case "ellipsis-end":
      return (text2) => clipEnd(text2, maxWidth, widthof, ellipsis$1);
  }
}
const ellipsis$1 = "…";
...
class Vector extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    const [cx, cy] = applyFrameAnchor(this, dimensions);
    return create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, { x: X3 && x2, y: Y3 && y2 }).call(
      (g) => g.selectAll().data(index).enter().append("path").call(applyDirectStyles, this).attr(
        "transform",
        template`translate(${X3 ? (i) => X3[i] : cx},${Y3 ? (i) => Y3[i] : cy})${A5 ? (i) => ` rotate(${A5[i]})` : rotate ? ` rotate(${rotate})` : ``}${anchor === "start" ? `` : anchor === "end" ? L ? (i) => ` translate(0,${L[i]})` : ` translate(0,${length2})` : L ? (i) => ` translate(0,${L[i] / 2})` : ` translate(0,${length2 / 2})`}`
      ).attr(
        "d",
        L ? (i) => {
          const p = pathRound();
...
function vector(data, options = {}) {
  let { x: x2, y: y2, ...rest } = options;
  if (options.frameAnchor === void 0) [x2, y2] = maybeTuple(x2, y2);
  return new Vector(data, { ...rest, x: x2, y: y2 });
}
  ...
  return marks(
    ...
    !isNoneish(fill) && label !== null ? text(
      ...
      labelOptions({ fill, fillOpacity, ...options }, function(data2, facets, channels, scales, dimensions) {
        ...
        return {
          facets: [[0]],
          channels: { text: { value: [formatAxisLabel(k2, scale2, { anchor, label, labelAnchor: cla, labelArrow })] } }
        };
      })
    ) : null
  );
...
function axisTextKy(k2, anchor, data, {
  ...
  return axisMark(
    ...
    function(scale2, data2, ticks2, tickFormat2, channels) {
      if (fontVariant === void 0) this.fontVariant = inferFontVariant(scale2);
      if (text2 === void 0) channels.text = inferTextChannel(scale2, data2, ticks2, tickFormat2, anchor);
    }
  );
}
...
function axisMark(mark, k2, data, properties, options, initialize) {
  ...
  function axisInitializer(data2, facets, _channels, scales, dimensions, context) {
    ...
    const { [k2]: scale2 } = scales;
    if (!scale2) throw new Error(`missing scale: ${k2}`);
    const domain = scale2.domain();
    let { interval: interval2, ticks: ticks2, tickFormat: tickFormat2, tickSpacing = k2 === "x" ? 80 : 35 } = options;
    if (typeof ticks2 === "string" && hasTemporalDomain(scale2)) interval2 = ticks2, ticks2 = void 0;
    if (ticks2 === void 0) ticks2 = maybeRangeInterval(interval2, scale2.type) ?? inferTickCount(scale2, tickSpacing);
    if (data2 == null) {
      if (isIterable(ticks2)) {
        ...
        if (scale2.ticks) {
          ...
          interval3 = generalizeTimeInterval(interval3, n / ticks2) ?? interval3;
          if (interval3 !== scale2.interval) data2 = inclusiveRange(interval3, ...extent$1(data2));
        }
        if (interval3 === scale2.interval) {
    ...
    initialize == null ? void 0 : initialize.call(this, scale2, data2, ticks2, tickFormat2, channels);
  ...
  const m2 = mark(data, initializer({ ...options, initializer: axisInitializer }, basicInitializer));
  if (data == null) {
  ...
  if (properties !== void 0) Object.assign(m2, properties);
  if (m2.clip === void 0) m2.clip = false;
  return m2;
}
function inferTickCount(scale2, tickSpacing) {
  const [min2, max2] = extent$1(scale2.range());
...
function inferTickFormat(scale2, data, ticks2, tickFormat2, anchor) {
  return typeof tickFormat2 === "function" && !(scale2.type === "log" && scale2.tickFormat) ? tickFormat2 : tickFormat2 === void 0 && data && isTemporal(data) ? inferTimeFormat(scale2.type, data, anchor) ?? formatDefault : scale2.tickFormat ? scale2.tickFormat(typeof ticks2 === "number" ? ticks2 : null, tickFormat2) : tickFormat2 === void 0 ? formatDefault : typeof tickFormat2 === "string" ? (isTemporal(scale2.domain()) ? utcFormat : format$1)(tickFormat2) : constant(tickFormat2);
}
...
function formatAxisLabel(k2, scale2, { anchor, label = scale2.label, labelAnchor, labelArrow } = {}) {
...
function legendItems(scale2, options = {}, swatch) {
  let {
    columns,
    tickFormat: tickFormat2,
    fontVariant = inferFontVariant$2(scale2),
    // TODO label,
    swatchSize = 15,
...
function legendOpacity({ type: type2, interpolate: interpolate2, ...scale2 }, { legend: legend2 = true, color: color2 = rgb(0, 0, 0), ...options }) {
  if (!interpolate2) throw new Error(`${type2} opacity scales are not supported`);
  if (legend2 === true) legend2 = "ramp";
  if (`${legend2}`.toLowerCase() !== "ramp") throw new Error(`${legend2} opacity legends are not supported`);
  return legendColor({ type: type2, ...scale2, interpolate: interpolateOpacity(color2) }, { legend: legend2, ...options });
}
...
function exclusiveFacets(data, facets) {
  ...
  O.fill(0);
  for (const facet of facets) {
    for (let k2 = 0, m2 = facet.length; k2 < m2; ++k2) {
      const i = facet[k2];
...
function stackX(stackOptions = {}, options = {}) {
  ...
  const [transform, Y3, x12, x22] = stack(y2, x2, "y", "x", stackOptions, rest);
  return { ...transform, y1: y12, y: Y3, x1: x12, x2: x22, x: mid(x12, x22) };
}
function stackX1(stackOptions = {}, options = {}) {
  if (arguments.length === 1) [stackOptions, options] = mergeOptions$2(stackOptions);
  const { y1: y12, y: y2 = y12, x: x2 } = options;
  const [transform, Y3, X3] = stack(y2, x2, "y", "x", stackOptions, options);
  return { ...transform, y1: y12, y: Y3, x: X3 };
}
...
function stackY(stackOptions = {}, options = {}) {
  ...
  const [transform, X3, y12, y22] = stack(x2, y2, "x", "y", stackOptions, rest);
  return { ...transform, x1: x12, x: X3, y1: y12, y2: y22, y: mid(y12, y22) };
}
function stackY1(stackOptions = {}, options = {}) {
  if (arguments.length === 1) [stackOptions, options] = mergeOptions$2(stackOptions);
  const { x1: x12, x: x2 = x12, y: y2 } = options;
  const [transform, X3, Y3] = stack(x2, y2, "x", "y", stackOptions, options);
  return { ...transform, x1: x12, x: X3, y: Y3 };
}
...
function maybeStackX({ x: x2, x1: x12, x2: x22, ...options } = {}) {
  ...
  if (x12 === void 0 && x22 === void 0) return stackX({ x: x2, ...options });
...
function stack(x2, y2 = one, kx2, ky2, { offset: offset2, order, reverse: reverse2 }, options) {
  ...
  return [
    basic(options, (data, facets, plotOptions) => {
      ...
      const X4 = x2 == null ? void 0 : setX(maybeApplyInterval(valueof(data, x2), plotOptions == null ? void 0 : plotOptions[kx2]));
      ...
      const facetstacks = [];
      for (const facet of facets) {
        const stacks = X4 ? Array.from(group$1(facet, (i) => X4[i]).values()) : [facet];
...
function offsetWiggle(facetstacks, Y12, Y22, Z) {
  for (const stacks of facetstacks) {
    ...
    for (const stack2 of stacks) {
      ...
      const s1 = sum$1(Fi);
      if (s1) y2 -= sum$1(Fi, (d, i) => (Df[i] / 2 + Cf1[i]) * d) / s1;
    }
    offsetZero(stacks, Y12, Y22);
  }
  offsetCenterFacets(facetstacks, Y12, Y22);
...
function offsetZero(stacks, Y12, Y22) {
  const m2 = min$2(stacks, (stack2) => min$2(stack2, (i) => Y12[i]));
  for (const stack2 of stacks) {
    for (const i of stack2) {
      Y12[i] -= m2;
      Y22[i] -= m2;
...
function offsetCenterFacets(facetstacks, Y12, Y22) {
  ...
  const m2 = facets.map((I) => (min$2(I, (i) => Y12[i]) + max$2(I, (i) => Y22[i])) / 2);
  const m0 = min$2(m2);
  for (let j = 0; j < n; j++) {
    const p = m0 - m2[j];
...
function orderZDomain(compare, domain) {
  return (data, X3, Y3, Z) => {
    ...
    const map2 = new InternMap(domain(data, X3, Y3, Z).map((d, i) => [d, i]));
...
class Rect extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    if ((X12 || X22) && !projection2 && isCollapsed(x2)) X12 = X22 = null;
    if ((Y12 || Y22) && !projection2 && isCollapsed(y2)) Y12 = Y22 = null;
    const bx = (x2 == null ? void 0 : x2.bandwidth) ? x2.bandwidth() : 0;
    const by = (y2 == null ? void 0 : y2.bandwidth) ? y2.bandwidth() : 0;
...
class Frame extends Mark {
  constructor(options = {}) {
    ...
    super(singleton, void 0, options, anchor == null ? defaults$g : lineDefaults);
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    return create(anchor ? "svg:line" : rx1y1 || rx1y2 || rx2y1 || rx2y2 ? "svg:path" : "svg:rect", context).datum(0).call(applyIndirectStyles, this, dimensions, context).call(applyDirectStyles, this).call(applyChannelStyles, this, channels).call(applyTransform, this, {}).call(
      anchor === "left" ? (line2) => line2.attr("x1", x12).attr("x2", x12).attr("y1", y12).attr("y2", y2) : anchor === "right" ? (line2) => line2.attr("x1", x2).attr("x2", x2).attr("y1", y12).attr("y2", y2) : anchor === "top" ? (line2) => line2.attr("x1", x12).attr("x2", x2).attr("y1", y12).attr("y2", y12) : anchor === "bottom" ? (line2) => line2.attr("x1", x12).attr("x2", x2).attr("y1", y2).attr("y2", y2) : rx1y1 || rx1y2 || rx2y1 || rx2y2 ? (path) => path.call(applyRoundedRect, x12, y12, x2, y2, this) : (rect2) => rect2.attr("x", x12).attr("y", y12).attr("width", x2 - x12).attr("height", y2 - y12).attr("rx", rx).attr("ry", ry)
    ).node();
  }
}
...
class Tip extends Mark {
  constructor(data, options = {}) {
    ...
    this.fontVariant = string(fontVariant);
    this.fontWeight = string(fontWeight);
    for (const key in defaults$f) if (key in this.channels) this[key] = defaults$f[key];
    this.splitLines = splitter(this);
    this.clipLine = clipper(this);
  ...
  render(index, scales, values2, dimensions, context) {
    ...
    const g = create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyIndirectTextStyles, this).call(applyTransform, this, { x: X3 && x2, y: Y3 && y2 }).call(
    ...
    function renderLine(selection2, { label, value, color: color2, opacity: opacity2 }) {
      ...
      if (j >= 0) {
        label = label.slice(0, j).trimEnd() + ellipsis$1;
        title = value.trim();
        value = "";
      } else {
        ...
        if (k2 >= 0) {
          title = value.trim();
          value = value.slice(0, k2).trimEnd() + ellipsis$1;
        }
      }
    ...
    function postrender() {
      ...
      g.selectChildren().each(function(i) {
        ...
        if (a2 === void 0) {
          const x3 = px(i) + ox2;
          const y3 = py(i) + oy2;
          const fitLeft = x3 + w + m2 + r * 2 < width;
          const fitRight = x3 - w - m2 - r * 2 > 0;
          const fitTop = y3 + h + m2 + r * 2 < height;
          const fitBottom = y3 - h - m2 - r * 2 > 0;
        ...
        path.setAttribute("d", getPath(a2, m2, r, w, h));
        ...
        text2.setAttribute("transform", `translate(${getTextTranslate(a2, m2, r, w, h)})`);
      });
...
function getTextTranslate(anchor, m2, r, width, height) {
  switch (anchor) {
    case "middle":
    ...
    case "top-left":
      return [r, m2 + r];
    case "top":
      return [-width / 2, m2 / 2 + r];
    case "top-right":
      return [-width - r, m2 + r];
    case "right":
      return [-m2 / 2 - width - r, height / 2];
    case "bottom-left":
      return [r, -m2 - r];
    case "bottom":
      return [-width / 2, -m2 / 2 - r];
    case "bottom-right":
      return [-width - r, -m2 - r];
    case "left":
      return [r + m2 / 2, height / 2];
  }
}
function getPath(anchor, m2, r, width, height) {
  const w = width + r * 2;
  const h = height + r * 2;
  switch (anchor) {
    case "middle":
    ...
    case "top-left":
      return `M0,0l${m2},${m2}h${w - m2}v${h}h${-w}z`;
    case "top":
      return `M0,0l${m2 / 2},${m2 / 2}h${(w - m2) / 2}v${h}h${-w}v${-h}h${(w - m2) / 2}z`;
    case "top-right":
      return `M0,0l${-m2},${m2}h${m2 - w}v${h}h${w}z`;
    case "right":
      return `M0,0l${-m2 / 2},${-m2 / 2}v${m2 / 2 - h / 2}h${-w}v${h}h${w}v${m2 / 2 - h / 2}z`;
    case "bottom-left":
      return `M0,0l${m2},${-m2}h${w - m2}v${-h}h${-w}z`;
    case "bottom":
      return `M0,0l${m2 / 2},${-m2 / 2}h${(w - m2) / 2}v${-h}h${-w}v${h}h${(w - m2) / 2}z`;
...
function plot(options = {}) {
  ...
  select$1(svg).attr("class", className).attr("fill", "currentColor").attr("font-family", "system-ui, sans-serif").attr("font-size", 10).attr("text-anchor", "middle").attr("width", width).attr("height", height).attr("viewBox", `0 0 ${width} ${height}`).attr("aria-label", ariaLabel).attr("aria-description", ariaDescription).call(
    (svg2) => (
      // Warning: if you edit this, change defaultClassName.
      svg2.append("style").text(
        `:where(.${className}) {
...
:where(.${className} tspan) {
  ...
  for (const mark of marks2) {
    ...
    if (facets === void 0 || mark.facet === "super") {
      ...
      for (const f of facets) {
        ...
        if (node == null) continue;
        (g ?? (g = select$1(svg).append("g"))).append(() => node).datum(f);
  ...
  if (w > 0) {
    select$1(svg).append("text").attr("x", width).attr("y", 20).attr("dy", "-1em").attr("text-anchor", "end").attr("font-family", "initial").text("⚠️").append("title").text(`${w.toLocaleString("en-US")} warning${w === 1 ? "" : "s"}. Please check the console.`);
  }
  return figure;
}
...
function applyScaleTransform(channel, options) {
  ...
  const {
    ...
    percent,
    interval: interval2,
    transform = percent ? (x2) => x2 == null ? NaN : x2 * 100 : maybeIntervalTransform(interval2, type2)
  } = options[scale2] ?? {};
...
function gridOptions(grid, {
  stroke = isColor$1(grid) ? grid : void 0,
...
function hasPositionChannel(k2, marks2) {
  for (const mark of marks2) {
    for (const key in mark.channels) {
      ...
      if (scale2 === k2 || scale2 === "projection") {
        return true;
      }
    }
  }
...
function outerRange(scale2) {
  ...
  return [x12, x2 + scale2.bandwidth()];
}
...
function binY(outputs = { x: "count" }, options = {}) {
  [outputs, options] = mergeOptions$1(outputs, options);
  const { x: x2, y: y2 } = options;
  return binn(null, maybeBinValue(y2, options, identity$1), x2, null, outputs, maybeInsetY(options));
}
...
function mergeOptions$1({ cumulative, domain, thresholds, interval: interval2, ...outputs }, options) {
...
function maybeBin(options) {
  ...
  const bin2 = (data) => {
    ...
    if (isTemporal(V) || isTimeThresholds(thresholds)) {
      V = map$1(V, coerceDate, Float64Array);
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    return create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, scales).call(
      (g) => g.selectAll().data(index).enter().append("path").call(applyDirectStyles, this).attr("d", (i) => {
        ...
        const bendAngle = this.sweep(x12, y12, x2, y2) * bend * radians;
...
function maybeSweep(sweep = 1) {
  ...
  if (typeof sweep === "function") return (x12, y12, x2, y2) => Math.sign(sweep(x12, y12, x2, y2));
  switch (keyword(sweep, "sweep", ["+x", "-x", "+y", "-y"])) {
    case "+x":
      return (x12, y12, x2) => ascending$2(x12, x2);
    case "-x":
      return (x12, y12, x2) => descending$1(x12, x2);
    case "+y":
      return (x12, y12, x2, y2) => ascending$2(y12, y2);
    case "-y":
      return (x12, y12, x2, y2) => descending$1(y12, y2);
  }
}
function pointPointCenter([ax, ay], [bx, by], r, sign2) {
  const dx = bx - ax, dy = by - ay, d = Math.hypot(dx, dy);
  const k2 = sign2 * Math.sqrt(r * r - d * d / 4) / d;
...
function arrow(data, { x: x2, x1: x12, x2: x22, y: y2, y1: y12, y2: y22, ...options } = {}) {
  [x12, x22] = maybeSameValue(x2, x12, x22);
  [y12, y22] = maybeSameValue(y2, y12, y22);
...
class AbstractBar extends Mark {
  constructor(data, channels, options = {}, defaults2 = barDefaults) {
  ...
  render(index, scales, channels, dimensions, context) {
    const { rx, ry, rx1y1, rx1y2, rx2y1, rx2y2 } = this;
    const x2 = this._x(scales, channels, dimensions);
    const y2 = this._y(scales, channels, dimensions);
    ...
    return create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(this._transform, this, scales).call(
      (g) => g.selectAll().data(index).enter().call(
        rx1y1 || rx1y2 || rx2y1 || rx2y2 ? (g2) => g2.append("path").call(applyDirectStyles, this).call(applyRoundedRect, x2, y2, add$1(x2, w), add$1(y2, h), this).call(applyChannelStyles, this, channels) : (g2) => g2.append("rect").call(applyDirectStyles, this).attr("x", x2).attr("width", w).attr("y", y2).attr("height", h).call(applyAttr, "rx", rx).call(applyAttr, "ry", ry).call(applyChannelStyles, this, channels)
      )
    ).node();
  }
...
class BarX extends AbstractBar {
  constructor(data, options = {}, defaults2) {
...
function cell(data, { x: x2, y: y2, ...options } = {}) {
  [x2, y2] = maybeTuple(x2, y2);
  return new Cell(data, { ...options, x: x2, y: y2 });
}
function cellX(data, { x: x2 = indexOf, fill, stroke, ...options } = {}) {
  if (fill === void 0 && maybeColorChannel(stroke)[0] === void 0) fill = identity$1;
  return new Cell(data, { ...options, x: x2, fill, stroke });
}
function cellY(data, { y: y2 = indexOf, fill, stroke, ...options } = {}) {
  if (fill === void 0 && maybeColorChannel(stroke)[0] === void 0) fill = identity$1;
  return new Cell(data, { ...options, y: y2, fill, stroke });
}
const defaults$a = {
  ariaLabel: "dot",
  fill: "none",
...
function dotX(data, { x: x2 = identity$1, ...options } = {}) {
  return new Dot(data, maybeIntervalMidY({ ...options, x: x2 }));
}
function dotY(data, { y: y2 = identity$1, ...options } = {}) {
  return new Dot(data, maybeIntervalMidX({ ...options, y: y2 }));
}
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    return create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, scales).call(
      (g) => g.selectAll().data(groupIndex(index, [X3, Y3], this, channels)).enter().append("path").call(applyDirectStyles, this).call(applyGroupedChannelStyles, this, channels).call(applyGroupedMarkers, this, channels, context).attr(
        "d",
        curve === curveAuto && context.projection ? sphereLine(context.projection, X3, Y3) : shapeLine().curve(curve).defined((i) => i >= 0).x((i) => X3[i]).y((i) => Y3[i])
      )
    ).node();
  }
}
...
function lineX(data, { x: x2 = identity$1, y: y2 = indexOf, ...options } = {}) {
  return new Line(data, maybeDenseIntervalY({ ...options, x: x2, y: y2 }));
}
function lineY(data, { x: x2 = indexOf, y: y2 = identity$1, ...options } = {}) {
  return new Line(data, maybeDenseIntervalX({ ...options, x: x2, y: y2 }));
}
function autoSpec(data, options) {
  options = normalizeOptions(options);
  const { x: x2, y: y2, color: color2, size } = options;
  const X3 = materializeValue(data, x2);
  const Y3 = materializeValue(data, y2);
  const C3 = materializeValue(data, color2);
  ...
  if (xReduce != null && yReduce != null) {
    ...
    if (X3 && Y3) {
      transformImpl = isOrdinal(X3) && isOrdinal(Y3) ? group : isOrdinal(X3) ? binY : isOrdinal(Y3) ? binX : bin;
    } else if (X3) {
      transformImpl = isOrdinal(X3) ? groupX : binX;
    } else if (Y3) {
      transformImpl = isOrdinal(Y3) ? groupY : binY;
...
function normalizeOptions({ x: x2, y: y2, color: color2, size, fx, fy, mark } = {}) {
  if (!isOptions(x2)) x2 = makeOptions(x2);
  if (!isOptions(y2)) y2 = makeOptions(y2);
  if (!isOptions(color2)) color2 = isColor$1(color2) ? { color: color2 } : makeOptions(color2);
  ...
  return { x: x2, y: y2, color: color2, size, fx, fy, mark };
}
...
function mapX(mapper, options = {}) {
  let { x: x2, x1: x12, x2: x22 } = options;
  if (x2 === void 0 && x12 === void 0 && x22 === void 0) options = { ...options, x: x2 = identity$1 };
  const outputs = {};
  if (x2 != null) outputs.x = mapper;
...
function mapY(mapper, options = {}) {
  ...
  if (y2 === void 0 && y12 === void 0 && y22 === void 0) options = { ...options, y: y2 = identity$1 };
  const outputs = {};
...
function map(outputs = {}, options = {}) {
  const z = maybeZ(options);
  const channels = Object.entries(outputs).map(([key, map2]) => {
    const input = maybeInput(key, options);
    if (input == null) throw new Error(`missing channel: ${key}`);
    const [output, setOutput] = column(input);
    return { key, input, output, setOutput, map: maybeMap(map2) };
  });
  return {
    ...basic(options, (data, facets) => {
      ...
      for (const facet of facets) {
        for (const I of Z ? group$1(facet, (i) => Z[i]).values() : [facet]) {
          channels.forEach(({ map: map2 }, i) => map2.mapIndex(I, X3[i], MX[i]));
        }
      }
      return { data, facets };
    }),
...
function maybeMap(map2) {
  if (map2 == null) throw new Error("missing map");
  if (typeof map2.mapIndex === "function") return map2;
  if (typeof map2.map === "function" && isObject(map2)) return mapMap(map2);
  if (typeof map2 === "function") return mapFunction(taker(map2));
  switch (`${map2}`.toLowerCase()) {
    case "cumsum":
      return mapCumsum;
    case "rank":
  ...
  throw new Error(`invalid map: ${map2}`);
}
function mapMap(map2) {
  console.warn("deprecated map interface; implement mapIndex instead.");
  return { mapIndex: map2.map.bind(map2) };
}
...
function window$1(options = {}) {
  ...
  return maybeReduce(reduce)(k2, maybeAnchor$1(anchor, k2), strict);
}
function maybeAnchor$1(anchor = "middle", k2) {
...
function maybeReduce(reduce = "mean") {
  if (typeof reduce === "string") {
    ...
    switch (reduce.toLowerCase()) {
      ...
      case "max":
        return reduceArray((I, V) => max$2(I, (i) => V[i]));
      case "mean":
        return reduceMean;
      case "median":
        return reduceAccessor(median);
      case "min":
        return reduceArray((I, V) => min$2(I, (i) => V[i]));
      case "mode":
...
function reduceSum(k2, s2, strict) {
  return strict ? {
    mapIndex(I, S, T) {
      let nans = 0;
      let sum2 = 0;
...
function reduceMean(k2, s2, strict) {
  if (strict) {
    const sum2 = reduceSum(k2, s2, strict);
    return {
      mapIndex(I, S, T) {
        sum2.mapIndex(I, S, T);
        for (let i = 0, n = I.length - k2 + 1; i < n; ++i) {
...
function reduceFirst(k2, s2, strict) {
  return strict ? {
    mapIndex(I, S, T) {
      for (let i = 0, n = I.length - k2; i < n; ++i) {
        T[I[i + s2]] = S[I[i]];
...
function reduceLast(k2, s2, strict) {
  return strict ? {
    mapIndex(I, S, T) {
      for (let i = 0, n = I.length - k2; i < n; ++i) {
        T[I[i + s2]] = S[I[i + k2 - 1]];
...
function bollingerX(data, {
  ...
  y: y2,
  k: k2 = defaults$8.k,
  color: color2 = defaults$8.color,
  opacity: opacity2 = defaults$8.opacity,
  fill = color2,
  fillOpacity = opacity2,
  stroke = color2,
  strokeOpacity,
  ...
  return marks(
    isNoneish(fill) ? null : areaX(
      ...
      map(
        ...
        { x1: x2, x2, y: y2, fill, fillOpacity, ...options }
      )
    ),
    isNoneish(stroke) ? null : lineX(data, map({ x: bollinger(options) }, { x: x2, y: y2, stroke, strokeOpacity, strokeWidth, ...options }))
  );
}
  ...
  return marks(
    isNoneish(fill) ? null : areaY(
      ...
      map(
        ...
        { x: x2, y1: y2, y2, fill, fillOpacity, ...options }
      )
    ),
    isNoneish(stroke) ? null : lineY(data, map({ y: bollinger(options) }, { x: x2, y: y2, stroke, strokeOpacity, strokeWidth, ...options }))
  );
}
function bollinger({ n = defaults$8.n, k: k2 = 0, strict = defaults$8.strict, anchor = defaults$8.anchor } = {}) {
  return window$1({ k: n, reduce: (Y3) => mean(Y3) + k2 * (deviation(Y3) || 0), strict, anchor });
}
const defaults$7 = {
  ariaLabel: "tick",
  fill: null,
...
function tickX(data, { x: x2 = identity$1, ...options } = {}) {
  return new TickX(data, { ...options, x: x2 });
}
function tickY(data, { y: y2 = identity$1, ...options } = {}) {
  return new TickY(data, { ...options, y: y2 });
}
function boxX(data, {
  x: x2 = identity$1,
  y: y2 = null,
  ...
  const group2 = y2 != null ? groupY : groupZ$1;
  return marks(
    ruleY(data, group2({ x1: loqr1, x2: hiqr2 }, { x: x2, y: y2, stroke, strokeOpacity, ...options })),
    barX(data, group2({ x1: "p25", x2: "p75" }, { x: x2, y: y2, fill, fillOpacity, ...options })),
    tickX(data, group2({ x: "p50" }, { x: x2, y: y2, stroke, strokeOpacity, strokeWidth, sort: sort2, ...options })),
    dot(data, map({ x: oqr }, { x: x2, y: y2, z: y2, r, stroke, strokeOpacity, ...options }))
  );
}
function boxY(data, {
  y: y2 = identity$1,
  ...
  const group2 = x2 != null ? groupX : groupZ$1;
  return marks(
    ruleX(data, group2({ y1: loqr1, y2: hiqr2 }, { x: x2, y: y2, stroke, strokeOpacity, ...options })),
    barY(data, group2({ y1: "p25", y2: "p75" }, { x: x2, y: y2, fill, fillOpacity, ...options })),
    tickY(data, group2({ y: "p50" }, { x: x2, y: y2, stroke, strokeOpacity, strokeWidth, sort: sort2, ...options })),
    dot(data, map({ y: oqr }, { x: x2, y: y2, z: x2, r, stroke, strokeOpacity, ...options }))
  );
}
...
function loqr1(values2) {
  ...
  return min$2(values2, (d) => d >= lo ? d : NaN);
}
function hiqr2(values2) {
  const hi = quartile3(values2) * 2.5 - quartile1(values2) * 1.5;
  return max$2(values2, (d) => d <= hi ? d : NaN);
}
function quartile1(values2) {
  return quantile$1(values2, 0.25);
}
function quartile3(values2) {
  return quantile$1(values2, 0.75);
}
const defaults$6 = {
  ariaLabel: "raster",
  stroke: null,
...
function number(input, name) {
  const x2 = +input;
  if (isNaN(x2)) throw new Error(`invalid ${name}: ${input}`);
  return x2;
}
function integer(input, name) {
  const x2 = Math.floor(input);
  if (isNaN(x2)) throw new Error(`invalid ${name}: ${input}`);
  return x2;
}
class AbstractRaster extends Mark {
  constructor(data, channels, options = {}, defaults2) {
    let {
      ...
      x: x2,
      y: y2,
      x1: x12 = x2 == null ? 0 : void 0,
      y1: y12 = y2 == null ? 0 : void 0,
      x2: x22 = x2 == null ? width : void 0,
      y2: y22 = y2 == null ? height : void 0,
      pixelSize = defaults2.pixelSize,
      blur = 0,
      interpolate: interpolate2
    } = options;
    if (width != null) width = integer(width, "width");
    if (height != null) height = integer(height, "height");
    if (x12 != null) x12 = number(x12, "x1");
    if (y12 != null) y12 = number(y12, "y1");
    if (x22 != null) x22 = number(x22, "x2");
    if (y22 != null) y22 = number(y22, "y2");
    if (x2 == null && (x12 == null || x22 == null)) throw new Error("missing x");
    if (y2 == null && (y12 == null || y22 == null)) throw new Error("missing y");
    if (data != null && width != null && height != null) {
      if (x2 === void 0 && x12 != null && x22 != null) x2 = denseX(x12, x22, width);
      if (y2 === void 0 && y12 != null && y22 != null) y2 = denseY(y12, y22, width, height);
    }
    super(
      data,
      {
    ...
    this.interpolate = x2 == null || y2 == null ? null : maybeInterpolate(interpolate2);
  }
}
class Raster extends AbstractRaster {
  constructor(data, options = {}) {
    ...
    super(data, void 0, options, defaults$6);
  ...
  render(index, scales, values2, dimensions, context) {
    var _a;
    const color2 = scales[(_a = values2.channels.fill) == null ? void 0 : _a.scale] ?? ((x3) => x3);
    const { x: X3, y: Y3 } = values2;
    const { document: document2 } = context;
    const [x12, y12, x2, y2] = renderBounds(values2, dimensions, context);
    const dx = x2 - x12;
    const dy = y2 - y12;
    ...
    if (this.interpolate) {
      const kx2 = w / dx;
      const ky2 = h / dy;
      const IX = map$1(X3, (x3) => (x3 - x12) * kx2, Float64Array);
      const IY = map$1(Y3, (y3) => (y3 - y12) * ky2, Float64Array);
    ...
    return create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, scales).call(
      (g2) => g2.append("image").attr("transform", `translate(${x12},${y12}) scale(${Math.sign(x2 - x12)},${Math.sign(y2 - y12)})`).attr("width", Math.abs(dx)).attr("height", Math.abs(dy)).attr("preserveAspectRatio", "none").call(applyAttr, "image-rendering", this.imageRendering).call(applyDirectStyles, this).attr("xlink:href", canvas.toDataURL())
    ).node();
  }
}
function maybeTuples(k2, data, options) {
  if (arguments.length < 3) options = data, data = null;
  let { x: x2, y: y2, [k2]: z, ...rest } = options;
  if (x2 === void 0 && y2 === void 0 && isTuples(data)) {
    x2 = first, y2 = second;
    if (z === void 0) z = third;
  }
  return [data, { ...rest, x: x2, y: y2, [k2]: z }];
}
function raster() {
  const [data, options] = maybeTuples("fill", ...arguments);
  return new Raster(
    data,
    data == null || options.fill !== void 0 || options.fillOpacity !== void 0 ? options : { ...options, fill: identity$1 }
  );
}
function renderBounds({ x1: x12, y1: y12, x2, y2 }, dimensions, { projection: projection2 }) {
...
function rasterBounds({ x1: x12, y1: y12, x2, y2 }, scales, dimensions, context) {
  const channels = {};
...
function sampler(name, options = {}) {
  ...
  return initializer({ ...options, [name]: void 0 }, function(data, facets, channels, scales, dimensions, context) {
    const { x: x2, y: y2 } = scales;
    if (!x2) throw new Error("missing scale: x");
    if (!y2) throw new Error("missing scale: y");
    ...
    for (const facet of facets ?? [void 0]) {
      for (let yi = 0.5; yi < h; ++yi) {
        for (let xi = 0.5; xi < w; ++xi, ++i) {
          V[i] = value(x2.invert(x12 + xi * kx2), y2.invert(y12 + yi * ky2), facet);
        }
      }
    }
...
function maybeInterpolate(interpolate2) {
  if (typeof interpolate2 === "function") return interpolate2;
  if (interpolate2 == null) return interpolateNone;
  switch (`${interpolate2}`.toLowerCase()) {
    case "none":
  ...
  throw new Error(`invalid interpolate: ${interpolate2}`);
}
...
function interpolatorBarycentric({ random = lcg(42) } = {}) {
  return (index, width, height, X3, Y3, V) => {
    ...
    for (let i = 0; i < triangles.length; i += 3) {
      ...
      const vb = V[index[tb]];
      const vc = V[index[tc]];
      for (let x3 = Math.floor(x12); x3 < x2; ++x3) {
        for (let y3 = Math.floor(y12); y3 < y2; ++y3) {
      ...
      if (!S[i]) {
        ...
        for (let l = 0; l < n; ++l) {
          ...
          if (rays[j](xp, yp)) {
            ...
            W[i] = mix(V.at(j - 1), t, V[j], 1 - t, V[j], 0, x2, y2);
            k2 = j;
            break;
          }
        }
...
function segmentProject(x12, y12, x2, y2, x3, y3) {
  const dx = x2 - x12;
  const dy = y2 - y12;
  const a2 = dx * (x2 - x3) + dy * (y2 - y3);
...
function interpolateNearest(index, width, height, X3, Y3, V) {
  ...
  let iy, ix;
  for (let y2 = 0.5, k2 = 0; y2 < height; ++y2) {
    ix = iy;
    for (let x2 = 0.5; x2 < width; ++x2, ++k2) {
      ix = delaunay.find(x2, y2, ix);
      if (x2 === 0.5) iy = ix;
      W[k2] = V[index[ix]];
    }
  }
    ...
    for (let y2 = 0.5, k2 = 0; y2 < height; ++y2) {
      ...
      for (let x2 = 0.5; x2 < width; ++x2, ++k2) {
        ...
        while ((distance = Math.hypot(X3[index[iw]] - cx, Y3[index[iw]] - cy)) > minDistance && step < maxSteps) {
          const angle = random(x2, y2, step) * 2 * Math.PI;
...
function blend(a2, ca, b, cb, c2, cc) {
  return ca * a2 + cb * b + cc * c2;
}
function pick(random) {
  return (a2, ca, b, cb, c2, cc, x2, y2) => {
    const u2 = random(x2, y2);
    return u2 < ca ? a2 : u2 < ca + cb ? b : c2;
  };
}
function mixer(F, random) {
  return isNumeric(F) || isTemporal(F) ? blend : pick(random);
}
function denseX(x12, x2, width) {
  return {
    transform(data) {
      const n = data.length;
      const X3 = new Float64Array(n);
      const kx2 = (x2 - x12) / width;
      const x02 = x12 + kx2 / 2;
      for (let i = 0; i < n; ++i) X3[i] = i % width * kx2 + x02;
      return X3;
    }
  };
}
function denseY(y12, y2, width, height) {
  return {
    transform(data) {
      const n = data.length;
      const Y3 = new Float64Array(n);
      const ky2 = (y2 - y12) / height;
      const y02 = y12 + ky2 / 2;
      for (let i = 0; i < n; ++i) Y3[i] = Math.floor(i / width) % height * ky2 + y02;
      return Y3;
    }
  };
}
...
class Contour extends AbstractRaster {
  constructor(data, { smooth = true, value, ...options } = {}) {
    ...
    if (value != null) {
      const v = { transform: (D2) => D2.map((d) => d.value), label: labelof(value) };
      for (const key in channels) {
    ...
    if (data == null) {
      ...
      let { interpolate: interpolate2 } = options;
      if (value === void 0) value = identity$1;
      if (interpolate2 === void 0) options.interpolate = "nearest";
    }
  ...
  filter(index, { x: x2, y: y2, value, ...channels }, values2) {
...
function contourGeometry({ thresholds, interval: interval2, ...options }) {
  ...
  return initializer(options, function(data, facets, channels, scales, dimensions, context) {
    const [x12, y12, x2, y2] = rasterBounds(channels, scales, dimensions, context);
    const dx = x2 - x12;
    const dy = y2 - y12;
    const { pixelSize: k2, width: w = Math.round(Math.abs(dx) / k2), height: h = Math.round(Math.abs(dy) / k2) } = this;
    const kx2 = w / dx;
    ...
    if (this.interpolate) {
      ...
      for (let i = 0; i < m2; ++i) VV.push(V.slice(i * n, i * n + n));
    } else {
      VV.push(V);
    }
    ...
    const { contour: contour2 } = contours().size([w, h]).smooth(this.smooth);
    const contourData = [];
    const contourFacets = [];
    for (const V2 of VV) {
      contourFacets.push(range$1(contourData.length, contourData.push(...map$1(T, (t) => contour2(V2, t)))));
    }
    for (const { coordinates } of contourData) {
      for (const rings of coordinates) {
        for (const ring of rings) {
          for (const point2 of ring) {
            point2[0] = point2[0] / kx2 + x12;
            point2[1] = point2[1] / ky2 + y12;
          }
        }
      }
    }
...
function maybeTicks(thresholds, V, min2, max2) {
  ...
  const tz = ticks(...nice$1(min2, max2, thresholds), thresholds);
...
function finiteExtent(VV) {
  return [min$2(VV, (V) => min$2(V, finite)), max$2(VV, (V) => max$2(V, finite))];
}
function finite(x2) {
  return isFinite(x2) ? x2 : NaN;
}
...
function crosshairK(pointer2, data, options = {}) {
  const { x: x2, y: y2, maxRadius } = options;
  const p = pointer2({ px: x2, py: y2, maxRadius });
  const M2 = [];
  if (x2 != null) M2.push(ruleX(data, ruleOptions("x", { ...p, inset: -6 }, options)));
  if (y2 != null) M2.push(ruleY(data, ruleOptions("y", { ...p, inset: -6 }, options)));
  if (x2 != null) M2.push(text(data, textOptions("x", { ...p, dy: 9, frameAnchor: "bottom", lineAnchor: "top" }, options)));
  if (y2 != null) M2.push(text(data, textOptions("y", { ...p, dx: -9, frameAnchor: "left", textAnchor: "end" }, options)));
  for (const m2 of M2) m2.ariaLabel = `crosshair ${m2.ariaLabel}`;
  return marks(...M2);
}
function markOptions(k2, { channels: pointerChannels, ...pointerOptions }, { facet, facetAnchor, fx, fy, [k2]: p, channels, transform, initializer: initializer2 }) {
  return {
    ...pointerOptions,
    facet,
    facetAnchor,
...
function pxpy(k2, i) {
  ...
  return function(data, facets, { x: x12, y: y12, px, py, ...c1 }, ...args) {
    const { channels: { x: x2, y: y2, ...c2 } = {}, ...rest } = i.call(this, data, facets, { ...c1, x: px, y: py }, ...args);
    return {
      channels: {
        ...c2,
        ...x2 && { px: x2, ...k2 === "x" && { x: x2 } },
...
class DelaunayLink extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    function links(index2) {
      ...
      select$1(this).selectAll().data(newIndex).enter().append("path").call(applyDirectStyles, mark).attr("d", (i2) => {
        const p = pathRound();
        const c2 = curve(p);
        c2.lineStart();
        c2.point(X12[i2], Y12[i2]);
        c2.point(X22[i2], Y22[i2]);
        c2.lineEnd();
        return p;
      }).call(applyChannelStyles, mark, newChannels).call(applyMarkers, mark, newChannels, context);
    }
    return create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, { x: X3 && x2, y: Y3 && y2 }).call(
      Z ? (g) => g.selectAll().data(group$1(index, (i) => Z[i]).values()).enter().append("g").each(links) : (g) => g.datum(index).each(links)
    ).node();
  }
}
class AbstractDelaunayMark extends Mark {
  constructor(data, options = {}, defaults2, zof = ({ z }) => z) {
    const { x: x2, y: y2 } = options;
    super(
      data,
      {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    function mesh(index2) {
      ...
      select$1(this).append("path").datum(index2[0]).call(applyDirectStyles, mark).attr("d", mark._render(delaunay, dimensions)).call(applyChannelStyles, mark, channels);
    }
    return create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, { x: X3 && x2, y: Y3 && y2 }).call(
      Z ? (g) => g.selectAll().data(group$1(index, (i) => Z[i]).values()).enter().append("g").each(mesh) : (g) => g.datum(index).each(mesh)
    ).node();
  }
}
...
class Voronoi2 extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    return create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, { x: X3 && x2, y: Y3 && y2 }).call((g) => {
      g.selectAll().data(index).enter().append("path").call(applyDirectStyles, this).attr("d", (i) => C3[i]).call(applyChannelStyles, this, channels);
    }).node();
  }
}
...
function voronoi(data, { x: x2, y: y2, initializer: initializer2, ...options } = {}) {
  return delaunayMark(Voronoi2, data, { ...basic({ ...options, x: x2, y: y2 }, exclusiveFacets), initializer: initializer2 });
}
...
class Density extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    const { contours: contours2 } = channels;
    const path = geoPath();
    return create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, {}).call(
      (g) => g.selectAll().data(index).enter().append("path").call(applyDirectStyles, this).call(applyChannelStyles, this, channels).attr("d", (i) => path(contours2[i]))
    ).node();
  }
}
function density(data, { x: x2, y: y2, ...options } = {}) {
  [x2, y2] = maybeTuple(x2, y2);
  return new Density(data, { ...options, x: x2, y: y2 });
}
const dropChannels = /* @__PURE__ */ new Set(["x", "y", "z", "weight"]);
function densityInitializer(options, fillDensity, strokeDensity) {
  ...
  return initializer(options, function(data, facets, channels, scales, dimensions, context) {
    ...
    const density2 = contourDensity().x(X3 ? (i) => X3[i] : cx).y(Y3 ? (i) => Y3[i] : cy).weight(W ? (i) => W[i] : 1).size([width, height]).bandwidth(bandwidth);
    ...
    for (const facetContours of facetsContours) {
      ...
      for (const [index, contour2] of facetContours) {
        for (const t of T) {
          newFacet.push(contours2.length);
          contours2.push(contour2(t / k2));
          if (FD) FD.push(t);
          if (SD) SD.push(t);
...
function differenceK(k2, data, {
  ...
  y1: y12,
  y2,
  x: x3 = x12 === void 0 && x2 === void 0 ? k2 === "y" ? indexOf : identity$1 : void 0,
  y: y3 = y12 === void 0 && y2 === void 0 ? k2 === "x" ? indexOf : identity$1 : void 0,
  fill,
  // ignored
  positiveFill = "#3ca951",
  negativeFill = "#4269d0",
  ...
  [x12, x2] = memoTuple(x3, x12, x2);
  [y12, y2] = memoTuple(y3, y12, y2);
  if (x12 === x2 && y12 === y2) {
    if (k2 === "y") y12 = memo(0);
    else x12 = memo(0);
  }
...
function memoTuple(x2, x12, x22) {
  if (x12 === void 0 && x22 === void 0) {
    x12 = x22 = memo(x2);
  } else if (x12 === void 0) {
    x22 = memo(x22);
    x12 = x2 === void 0 ? x22 : memo(x2);
  } else if (x22 === void 0) {
    x12 = memo(x12);
    x22 = x2 === void 0 ? x12 : memo(x2);
  } else {
    x12 = memo(x12);
    x22 = memo(x22);
  }
...
function clipDifference(k2, positive2) {
  ...
  return (index, scales, channels, dimensions, context, next) => {
    ...
    const m2 = dimensions[k2 === "y" ? "height" : "width"];
    (positive2 === inferScaleOrder(scales[k2]) < 0 ? K1 : K2).fill(m2);
    ...
    const c2 = oc.querySelector("g") ?? oc;
    const g = og.querySelector("g") ?? og;
    for (let i = 0; c2.firstChild; i += 2) {
      const id2 = getClipId();
      const clipPath = create("svg:clipPath", context).attr("id", id2).node();
      clipPath.appendChild(c2.firstChild);
...
function centroid({ geometry = identity$1, ...options } = {}) {
  ...
  return initializer(
    ...
    (data, facets, channels, scales, dimensions, { projection: projection2 }) => {
      ...
      const Y3 = new Float64Array(n);
      const path = geoPath(projection2);
      for (let i = 0; i < n; ++i) [X3[i], Y3[i]] = path.centroid(G[i]);
      return {
        data,
        facets,
        channels: {
          x: { value: X3, scale: projection2 == null ? "x" : null, source: null },
          y: { value: Y3, scale: projection2 == null ? "y" : null, source: null }
        }
      };
    }
  );
...
function geoCentroid({ geometry = identity$1, ...options } = {}) {
  ...
  return {
    ...options,
    x: { transform: (data) => Float64Array.from(getC(data), ([x2]) => x2) },
    y: { transform: (data) => Float64Array.from(getC(data), ([, y2]) => y2) },
...
function scaleProjection({ x: X3, y: Y3 }) {
  if (X3 || Y3) {
    X3 ?? (X3 = (x2) => x2);
    Y3 ?? (Y3 = (y2) => y2);
    return geoTransform({
...
function hexbin(outputs = { fill: "count" }, { binWidth, ...options } = {}) {
  const { z } = options;
  binWidth = binWidth === void 0 ? 20 : number$1(binWidth);
  ...
  return initializer(options, (data, facets, channels, scales, _, context) => {
    ...
    for (const facet of facets) {
      ...
      for (const o of outputs) o.scope("facet", facet);
      for (const [f, I] of maybeGroup(facet, G)) {
        for (const { index: b, extent: extent2 } of hbin(data, I, X3, Y3, binWidth)) {
          binFacet.push(++i);
          BX.push(extent2.x);
          BY.push(extent2.y);
...
class Hexgrid extends Mark {
  constructor({ binWidth = 20, clip: clip2 = true, ...options } = {}) {
    super(singleton, void 0, { clip: clip2, ...options }, defaults$2);
    this.binWidth = number$1(binWidth);
  }
  render(index, scales, channels, dimensions, context) {
    const { binWidth } = this;
    const { marginTop, marginRight, marginBottom, marginLeft, width, height } = dimensions;
    const x02 = marginLeft - ox, x12 = width - marginRight - ox, y02 = marginTop - oy, y12 = height - marginBottom - oy, rx = binWidth / 2, ry = rx * sqrt4_3, hy = ry / 2, wx = rx * 2, wy = ry * 1.5, i0 = Math.floor(x02 / wx), i1 = Math.ceil(x12 / wx), j0 = Math.floor((y02 + hy) / wy), j1 = Math.ceil((y12 - hy) / wy) + 1, path = `m0,${round(-ry)}l${round(rx)},${round(hy)}v${round(ry)}l${round(-rx)},${round(hy)}`;
    let d = path;
    for (let j = j0; j < j1; ++j) {
      for (let i = i0; i < i1; ++i) {
...
class Image extends Mark {
  constructor(data, options = {}) {
    let { x: x2, y: y2, r, width, height, rotate, src: src2, preserveAspectRatio, crossOrigin, frameAnchor, imageRendering } = options;
    if (r == null) r = void 0;
    ...
    const [vs, cs] = maybePathChannel(src2);
...
function ibetainv(p, a2, b) {
  ...
  var b1 = b - 1;
  var j = 0;
  var lna, lnb, pp, t, u2, err, x2, al, h, w, afac;
  if (p <= 0) return 0;
  if (p >= 1) return 1;
  if (a2 >= 1 && b >= 1) {
    pp = p < 0.5 ? p : 1 - p;
    ...
    u2 = Math.exp(b * lnb) / b;
    w = t + u2;
    if (p < t / w) x2 = Math.pow(a2 * w * p, 1 / a2);
    else x2 = 1 - Math.pow(b * w * (1 - p), 1 / b);
  }
  ...
  for (; j < 10; j++) {
    ...
    u2 = err / t;
    x2 -= t = u2 / (1 - 0.5 * Math.min(1, u2 * (a1 / x2 - b1 / (1 - x2))));
    if (x2 <= 0) x2 = 0.5 * (x2 + t);
    if (x2 >= 1) x2 = 0.5 * (x2 + t + 1);
    if (Math.abs(t) < EPS * x2 && j > 0) break;
  }
  return x2;
}
...
class LinearRegression extends Mark {
  ...
  render(index, scales, channels, dimensions, context) {
    ...
    const { ci } = this;
    return create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, scales).call(
      (g) => g.selectAll().data(Z ? groupZ(index, Z, this.z) : [index]).enter().call(
...
class LinearRegressionX extends LinearRegression {
  ...
  _renderBand(I, X3, Y3) {
    ...
    const [y12, y2] = extent$1(I, (i) => Y3[i]);
    const f = linearRegressionF(I, Y3, X3);
    const g = confidenceIntervalF(I, Y3, X3, (1 - ci) / 2, f);
    return shapeArea().y((y3) => y3).x0((y3) => g(y3, -1)).x1((y3) => g(y3, 1))(range$1(y12, y2 - precision / 2, precision).concat(y2));
  }
  _renderLine(I, X3, Y3) {
    const [y12, y2] = extent$1(I, (i) => Y3[i]);
    const f = linearRegressionF(I, Y3, X3);
...
class LinearRegressionY extends LinearRegression {
  ...
  _renderBand(I, X3, Y3) {
    const { ci, precision } = this;
    const [x12, x2] = extent$1(I, (i) => X3[i]);
    const f = linearRegressionF(I, X3, Y3);
    const g = confidenceIntervalF(I, X3, Y3, (1 - ci) / 2, f);
    return shapeArea().x((x3) => x3).y0((x3) => g(x3, -1)).y1((x3) => g(x3, 1))(range$1(x12, x2 - precision / 2, precision).concat(x2));
  }
  _renderLine(I, X3, Y3) {
    const [x12, x2] = extent$1(I, (i) => X3[i]);
    const f = linearRegressionF(I, X3, Y3);
...
function linearRegressionX(data, { y: y2 = indexOf, x: x2 = identity$1, stroke, fill = isNoneish(stroke) ? "currentColor" : stroke, ...options } = {}) {
  return new LinearRegressionX(data, maybeDenseIntervalY({ ...options, x: x2, y: y2, fill, stroke }));
}
function linearRegressionY(data, { x: x2 = indexOf, y: y2 = identity$1, stroke, fill = isNoneish(stroke) ? "currentColor" : stroke, ...options } = {}) {
...
function confidenceIntervalF(I, X3, Y3, p, f) {
  const mean2 = sum$1(I, (i) => X3[i]) / I.length;
  let a2 = 0, b = 0;
  for (const i of I) {
    a2 += (X3[i] - mean2) ** 2;
    b += (Y3[i] - f(X3[i])) ** 2;
...
function maybeTreeSort(sort2) {
  return sort2 == null || typeof sort2 === "function" ? sort2 : `${sort2}`.trim().toLowerCase().startsWith("node:") ? nodeSort(maybeNodeValue(sort2)) : nodeSort(nodeData(sort2));
}
...
class WaffleX extends BarX {
  constructor(data, { unit: unit2 = 1, gap = 1, round: round2, render: render2, multiple, ...options } = {}) {
    super(data, { ...options, render: composeRender(render2, waffleRender("x")) }, waffleDefaults);
    this.unit = Math.max(0, unit2);
    this.gap = +gap;
    this.round = maybeRound(round2);
...
function waffleRender(y2) {
  return function(index, scales, values2, dimensions, context) {
    const { unit: unit2, gap, rx, ry, round: round2 } = this;
    const { document: document2 } = context;
    const Y12 = values2.channels[`${y2}1`].value;
    const Y22 = values2.channels[`${y2}2`].value;
    const barwidth = this[y2 === "y" ? "_width" : "_height"](scales, values2, dimensions);
    const barx = this[y2 === "y" ? "_x" : "_y"](scales, values2, dimensions);
    const scale2 = unit2 * scaleof(scales.scales[y2]);
    ...
    const transform = y2 === "y" ? ([x2, y3]) => [x2 * cx, -y3 * cy] : ([x2, y3]) => [y3 * cy, x2 * cx];
    const tx = (barwidth - multiple * cx) / 2;
    const x02 = typeof barx === "function" ? (i) => barx(i) + tx : barx + tx;
    const y02 = scales[y2](0);
    const patternId = getPatternId();
    const basePattern = document2.createElementNS(namespaces.svg, "pattern");
    basePattern.setAttribute("width", y2 === "y" ? cx : cy);
    basePattern.setAttribute("height", y2 === "y" ? cy : cx);
    basePattern.setAttribute("patternUnits", "userSpaceOnUse");
    const basePatternRect = basePattern.appendChild(document2.createElementNS(namespaces.svg, "rect"));
    basePatternRect.setAttribute("x", gap / 2);
    basePatternRect.setAttribute("y", gap / 2);
    basePatternRect.setAttribute("width", (y2 === "y" ? cx : cy) - gap);
    basePatternRect.setAttribute("height", (y2 === "y" ? cy : cx) - gap);
    if (rx != null) basePatternRect.setAttribute("rx", rx);
    if (ry != null) basePatternRect.setAttribute("ry", ry);
    return create("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(this._transform, this, scales).call(
      (g) => g.selectAll().data(index).enter().append(() => basePattern.cloneNode(true)).attr("id", (i) => `${patternId}-${i}`).select("rect").call(applyDirectStyles, this).call(applyChannelStyles, this, values2)
    ).call(
      (g) => g.selectAll().data(index).enter().append("path").attr("transform", y2 === "y" ? template`translate(${x02},${y02})` : template`translate(${y02},${x02})`).attr(
        "d",
        (i) => `M${wafflePoints(round2(Y12[i] / unit2), round2(Y22[i] / unit2), multiple).map(transform).join("L")}Z`
...
function scaleof({ domain, range: range2 }) {
  return spread(range2) / spread(domain);
}
...
function ge(a2, y2, c2, l, h) {
  var i = h + 1;
  while (l <= h) {
    var m2 = l + h >>> 1, x2 = a2[m2];
    var p = c2 !== void 0 ? c2(x2, y2) : x2 - y2;
    if (p >= 0) {
      i = m2;
      h = m2 - 1;
    } else {
...
function norm(a2, y2, c2, l, h, f) {
  if (typeof c2 === "function") {
    return f(a2, y2, c2, l === void 0 ? 0 : l | 0, h === void 0 ? a2.length - 1 : h | 0);
  }
  return f(a2, y2, void 0, c2 === void 0 ? 0 : c2 | 0, l === void 0 ? a2.length - 1 : l | 0);
}
var searchBounds = {
  ge: function(a2, y2, c2, l, h) {
    return norm(a2, y2, c2, l, h, ge);
  },
  gt: function(a2, y2, c2, l, h) {
    return norm(a2, y2, c2, l, h, gt);
  },
  lt: function(a2, y2, c2, l, h) {
    return norm(a2, y2, c2, l, h, lt);
  },
  le: function(a2, y2, c2, l, h) {
    return norm(a2, y2, c2, l, h, le);
  },
  eq: function(a2, y2, c2, l, h) {
    return norm(a2, y2, c2, l, h, eq);
  }
};
...
const IntervalTree$1 = /* @__PURE__ */ getDefaultExportFromCjs(intervalTree);
...
function dodge(y2, x2, anchor, padding, r, options) {
  if (r != null && typeof r !== "number") {
    let { channels, sort: sort2, reverse: reverse2 } = options;
    channels = maybeNamed(channels);
    if ((channels == null ? void 0 : channels.r) === void 0) options = { ...options, channels: { ...channels, r: { value: r, scale: "r" } } };
    if (sort2 === void 0 && reverse2 === void 0) options.sort = { channel: "-r" };
  }
  return initializer(options, function(data, facets, channels, scales, dimensions, context) {
    let { [x2]: X3, r: R } = channels;
    if (!channels[x2]) throw new Error(`missing channel: ${x2}`);
    ({ [x2]: X3 } = applyPosition(channels, scales, context));
    const cr = R ? void 0 : r !== void 0 ? number$1(r) : this.r !== void 0 ? this.r : 3;
    ...
    for (let I of facets) {
      const tree2 = IntervalTree$1();
      I = I.filter(R ? (i) => finite$1(X3[i]) && positive(R[i]) : (i) => finite$1(X3[i]));
      const intervals = new Float64Array(2 * I.length + 2);
      for (const i of I) {
        const ri = radius2(i);
        const y02 = ky2 ? ri + padding : 0;
        const l = X3[i] - ri;
        const h = X3[i] + ri;
        let k2 = 2;
        ...
        if (ky2) candidates = candidates.filter((y3) => y3 >= 0);
        out: for (const y3 of candidates.sort(compare)) {
    ...
    return {
      ...
      facets,
      channels: {
        [y2]: { value: Y3, source: null },
        // don’t show in tooltip
        [x2]: { value: X3, source: channels[x2] },
...
function normalizeX(basis2, options) {
  ...
  return mapX(normalize(basis2), options);
}
function normalizeY(basis2, options) {
  if (arguments.length === 1) ({ basis: basis2, ...options } = basis2);
  return mapY(normalize(basis2), options);
}
function normalize(basis2) {
...
const normalizeDeviation = {
  mapIndex(I, S, T) {
    const m2 = mean(I, (i) => S[i]);
    const d = deviation(I, (i) => S[i]);
    for (const i of I) {
      T[i] = S[i] === null ? NaN : d ? (S[i] - m2) / d : 0;
    }
  }
};
const normalizeMax = normalizeAccessor(max$2);
const normalizeMean = normalizeAccessor(mean);
const normalizeMedian = normalizeAccessor(median);
const normalizeMin = normalizeAccessor(min$2);
...
function shiftK(x2, interval2, options = {}) {
  let offset2;
  let k2 = 1;
  ...
  mapped[x22].transform = () => {
    const V = t();
    const [x02, x13] = extent$1(V);
    V.domain = k2 < 0 ? [x02, offset2(x13, k2)] : [offset2(x02, k2), x13];
    return V;
  };
  return mapped;
}
function select(selector2, options = {}) {
  if (typeof selector2 === "string") {
    switch (selector2.toLowerCase()) {
      case "first":
  ...
  if (typeof selector2 === "function") {
    return selectChannel(null, selector2, options);
  }
  let key, value;
  for (key in selector2) {
    if (value !== void 0) throw new Error("ambiguous selector; multiple inputs");
    value = maybeSelector(selector2[key]);
  }
  if (value === void 0) throw new Error(`invalid selector: ${selector2}`);
  return selectChannel(key, value, options);
}
function maybeSelector(selector2) {
  if (typeof selector2 === "function") return selector2;
  switch (`${selector2}`.toLowerCase()) {
    case "min":
      return selectorMin;
    case "max":
  ...
  throw new Error(`unknown selector: ${selector2}`);
}
...
function selectChannel(v, selector2, options) {
  if (v != null) {
...
const Plot = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  Area,
  Arrow,
  BarX,
...
function prepareCirclePackData(data, instance) {
  if (!data) return [];
  const { width, height } = instance.derivePlotOptions();
  const total = sum$1(data, (d) => d.y || 0);
  const groupedData = Array.from(
    ...
    ([key, values2]) => ({
      name: key,
      children: values2.map((v) => ({
        name: v.series,
        // Consider renaming to "category"
        ...v,
  ...
  }).sum((d) => d.y || 0).sort((a2, b) => b.y - a2.y);
  pack().size([width ?? 500, height ?? 500]).padding(3)(root2);
  return root2;
}
function getDataOrder(data, column2) {
  if (!data) return;
  return { domain: [...new Set(data.map((d) => d[column2]))] };
}
function getSorts(instance, columns, inputData) {
  var _a, _b, _c;
  const haveSorts = Object.keys(((_b = (_a = instance.mark()) == null ? void 0 : _a.options) == null ? void 0 : _b.sort) ?? {});
  const currentColumns = columns ? columns : instance.data().types ? Object.keys(instance.data().types ?? {}).filter(
    (d) => d !== "fy" && !haveSorts.includes(d)
  ) : [];
  const categoricalSeries = ((_c = instance.color().options) == null ? void 0 : _c.type) === "categorical";
  const data = inputData ?? instance.data();
  return currentColumns.filter(
    (column2) => data && data.types && (data == null ? void 0 : data.types[column2]) === "string" || column2 === "series" && categoricalSeries
  ).reduce((acc, column2) => {
    acc[column2] = getDataOrder(data, column2);
    return acc;
  }, {});
}
const defaultConfig = {
...
function getPlotOptions(instance) {
  ...
  const currentColumns = (data == null ? void 0 : data.types) ? Object.keys(data.types ?? {}) : [];
  const markType = instance.mark().type;
  const xDomain = (sorts == null ? void 0 : sorts.x) ? sorts.x : currentColumns.includes("y") || markType === "treemap" || markType === "circlePack" ? {} : {
    domain: extent$1(
      [...data, ...[((_a = data == null ? void 0 : data.types) == null ? void 0 : _a.x) === "number" ? { x: 0 } : {}]],
      (d) => d.x
    )
  };
  const yDomain = (sorts == null ? void 0 : sorts.y) ? sorts.y : currentColumns.includes("x") || markType === "treemap" || markType === "circlePack" ? {} : {
    domain: extent$1(
      [...data, ...[((_b = data == null ? void 0 : data.types) == null ? void 0 : _b.y) === "number" ? { y: 0 } : {}]],
      (d) => d.y
    )
  };
  const { color: colorConfig } = options;
  const { domain: sortsDomainRaw } = (sorts == null ? void 0 : sorts.series) || {};
  const sortsDomain = sortsDomainRaw ? sortsDomainRaw : ((_c = data == null ? void 0 : data.types) == null ? void 0 : _c.series) === "number" || ((_d = data == null ? void 0 : data.types) == null ? void 0 : _d.series) === "date" ? extent$1(
    [...data, ...[((_e = data == null ? void 0 : data.types) == null ? void 0 : _e.series) === "number" ? { series: 0 } : {}]],
    (d) => d.series
  ) : void 0;
  let colorDomain, colorRange, colorScheme;
  const categoricalColor = ((_f = data == null ? void 0 : data.types) == null ? void 0 : _f.series) === "string" || !Array.isArray(options.color) && ((_g = options.color) == null ? void 0 : _g.type) === "categorical";
  if (Array.isArray(colorConfig)) {
    colorRange = colorConfig;
    colorDomain = sortsDomain;
  } else if (typeof colorConfig === "object" && (colorConfig.domain !== void 0 || colorConfig.range !== void 0 || colorConfig.scheme !== void 0)) {
    colorDomain = colorConfig.domain || sortsDomain;
    colorRange = colorConfig.range;
    colorScheme = colorConfig.scheme;
  } else {
    colorDomain = sortsDomain;
    colorRange = categoricalColor ? defaultColors : void 0;
    colorScheme = !categoricalColor ? "RdPu" : void 0;
  }
  const hasColor = currentColumns.includes("series") || colorConfig;
  const computedColor = hasColor ? {
    label: Array.isArray(options.color) ? "" : (_h = options.color) == null ? void 0 : _h.label,
    ...colorDomain && { domain: colorDomain },
    ...colorRange && { range: colorRange },
    ...colorScheme && { scheme: colorScheme }
  } : {};
  const computedX = currentColumns.includes("fx") || markType === "treemap" || markType === "circlePack" ? { axis: null, ...xDomain } : {
    tickSize: 0,
    tickPadding: 5,
    ...!(config == null ? void 0 : config.xLabelDisplay) || !((_i = options.x) == null ? void 0 : _i.label) ? { labelArrow: "none" } : {},
    ...currentColumns.includes("x") && getTickFormatter(
      (_j = data == null ? void 0 : data.types) == null ? void 0 : _j.x,
      "x",
      options.width || 0,
      options.height || 0
    ),
  ...
  const computedY = markType === "treemap" || markType === "circlePack" ? { axis: null } : {
    labelArrow: !(config == null ? void 0 : config.yLabelDisplay) || !((_k = options.y) == null ? void 0 : _k.label) ? "none" : true,
    labelAnchor: "top",
    tickSize: 0,
    tickPadding: 5,
    ...currentColumns.includes("y") && getTickFormatter(
      (_l = data == null ? void 0 : data.types) == null ? void 0 : _l.y,
      "y",
      options.width || 0,
      options.height || 0
    ),
  ...
  const x2 = markType === "circlePack" ? {
    ...
    ...computedX,
    ...options.x,
    label: !(config == null ? void 0 : config.xLabelDisplay) ? null : (_m = options.x) == null ? void 0 : _m.label
  };
  const y2 = markType === "circlePack" ? {
    axis: null,
    ...
    ...computedY,
    ...options.y,
    label: !(config == null ? void 0 : config.yLabelDisplay) ? null : (_n = options.y) == null ? void 0 : _n.label
  };
  const r = markType === "circlePack" ? {
    range: [
      0,
      max$2(
  ...
  return {
    ...
    ...currentColumns.includes("fy") ? {
      fy: { ...sorts == null ? void 0 : sorts.fy, axis: null, label: null, ...options.fy },
      insetTop: options.insetTop || 12
    } : {},
    // This is based on the assumption that fx comes from a groupedBar chart
    ...currentColumns.includes("fx") ? { fx: { ...sorts == null ? void 0 : sorts.fx, label: (_o = options.x) == null ? void 0 : _o.label } } : {}
  };
}
function truncateText(text2, direction, width, height) {
  const size = direction === "y" ? width : height;
  const fixedMax = 30;
  const maxCharacters = Math.min(Math.floor(size * 0.2 / 3), fixedMax);
  if (text2.length > maxCharacters) {
    return text2.substring(0, maxCharacters) + "…";
  }
  return text2;
}
function getTickFormatter(colType, direction, width, height) {
  if (colType === "string") {
    return {
      tickFormat: (value) => {
...
function getLegendType(data) {
  if (!data.types) return;
  return data.types.series === "string" ? "categorical" : "continuous";
}
function getCommonMarks(currentColumns, inputOptions) {
  ...
  return [
    ...
    ...[
      (currentColumns == null ? void 0 : currentColumns.includes("y")) ? gridY({
        stroke: options.borderColor,
...
function getfyMarks(data, currentColumns, options) {
  return currentColumns.includes("fy") && !(Array.isArray(options == null ? void 0 : options.ticks) && (options == null ? void 0 : options.ticks.length) === 0) ? [
    text(
      data,
      selectFirst({
        text: (d) => d.fy,
...
async function checkDistinct(duckDB, tableName, cols) {
  var _a;
  if (!duckDB || !tableName || !cols || !cols.length) return false;
  const query = `SELECT CASE WHEN count(distinct(${(_a = quoteColumns(cols)) == null ? void 0 : _a.join(
    ", "
  )}))= count(*)
...
function allowAggregation(chartType) {
...
const columnTypes = async (db, name) => {
  ...
  types.forEach((type2) => {
    columns.set(type2.column_name, type2.data_type);
  });
  return columns;
};
function formatResults(data, schema) {
  let types = {};
  schema.forEach(
  ...
  const selected = data;
  let formatted = [];
  formatted = (selected == null ? void 0 : selected.map((row) => {
    let obj = {};
...
function coerceValue(value, columnType) {
  switch (columnType) {
    case "date":
...
function getTypeCategory(type2) {
  if (!type2) return void 0;
  const typeUpper = type2.toUpperCase();
  const isNumber = (typeUpper == null ? void 0 : typeUpper.startsWith("DECIMAL")) || (typeUpper == null ? void 0 : typeUpper.startsWith("NUMERIC")) || numberTypes.includes(typeUpper);
  return isNumber ? "number" : dateTypes.includes(typeUpper) ? "date" : "string";
}
const numberTypes = [
  "BIGINT",
  "INT8",
...
function processRawData(instance) {
  const rawData = instance.rawData();
  if (!rawData || !rawData.types) return [];
  const colIsDefined = (key, col) => !(key === "series" && isColor(col)) && col !== "" && col !== void 0 && typeof col === "string";
  ...
  const dataArray = rawData.map(
    (d) => Object.fromEntries(
      columnMappings.filter(({ key, column: column2 }) => colIsDefined(key, column2)).map(({ key, column: column2 }) => [key, d[column2]])
    )
  );
  const dataTypes = Object.fromEntries(
    columnMappings.filter(({ key, column: column2 }) => colIsDefined(key, column2)).map(({ key, column: column2 }) => {
      var _a;
      return [key, (_a = rawData == null ? void 0 : rawData.types) == null ? void 0 : _a[column2]];
    })
  );
  const dataLabels = Object.fromEntries(
    columnMappings.filter(({ column: column2 }) => column2).map(({ key, column: column2 }) => [key, column2])
  );
  dataArray.types = dataTypes;
  dataArray.labels = dataLabels;
  return dataArray;
}
function filterData(data, visibleSeries, seriesDomain) {
  return visibleSeries && visibleSeries.length > 0 ? data.filter((d) => visibleSeries.includes(`${d.series}`)) : seriesDomain && seriesDomain.length === 2 ? data.filter(
...
const checkForConfigErrors = (instance) => {
  const rawData = instance.rawData();
  ...
  const type2 = instance.mark().type;
  if (!type2 && !instance.markColumn().column)
    throw new Error("Mark type or mark column not set");
  const multipleX = Array.isArray(instance.x().column) && instance.x().column.length > 1;
  const multipleY = Array.isArray(instance.y().column) && instance.y().column.length > 1;
  const multipleColor = Array.isArray(instance.color().column) && instance.color().column.length > 1;
  if (type2 === "treemap" || type2 === "circlePack") {
    if (multipleX)
      throw new Error(`Multiple x columns not supported for ${type2} type`);
    if (multipleY)
      throw new Error(`Multiple y columns not supported for ${type2} type`);
    if (multipleColor)
      throw new Error(`Multiple color columns not supported for ${type2} type`);
    if (instance.fx().column || instance.fy().column)
      throw new Error(`Faceting not supported for ${type2} type`);
  } else if (type2 === "barX" || type2 === "rectX") {
    if (multipleY)
      throw new Error(
...
async function prepareData(instance) {
  let queries = {};
  ...
  const columns = Object.fromEntries(
    ...
    // Remove `false` or `undefined` entries
  );
...
`;
  ...
  let transformTableFrom = preQuery ? preQueryTableName : `${catalogPrefix}${instance.table()}`;
  const initialSchema = await runQuery(
    instance.ddb,
    `DESCRIBE ${transformTableFrom}`
  );
  const allColumns = Object.entries(columns).flatMap(([key, col]) => col);
  const schemaCols = initialSchema.map((row) => row.column_name);
  ...
  let distinctCols = (type2 === "barX" || type2 === "rectX" ? ["y", "series", "fy", "fx", "text"] : ["x", "series", "fy", "fx", "text"]).filter((d) => columnIsDefined(d, columns));
  const yValue = Array.isArray(columns.y) ? columns.y.filter((d) => d) : [columns.y];
  ...
  const allowsAggregation = allowAggregation(type2) || instance.config().aggregate;
  const shouldAggregate = !isDistinct && allowsAggregation && (distinctCols.includes("y") || distinctCols.includes("x") || distinctCols.includes("fx") || (type2 === "treemap" || type2 === "circlePack" || type2 === "pie") && (distinctCols.includes("series") || distinctCols.includes("text")) || instance.config().aggregate);
  ...
  let formatted = formatResults(data, schema);
  if (!labels.series) {
  ...
  return {
    data: formatted,
    description: description.value || "No transformations or aggregations applied.",
    queries
  };
}
...
function derivePlotOptions(instance) {
  var _a, _b, _c, _d;
  const data = instance.data();
  ...
  if (plotOptions.x.label === void 0) plotOptions.x.label = (_a = data.labels) == null ? void 0 : _a.x;
  if (plotOptions.y.label === void 0) plotOptions.y.label = (_b = data.labels) == null ? void 0 : _b.y;
  if (plotOptions.color.label === void 0)
    plotOptions.color.label = (_c = data.labels) == null ? void 0 : _c.series;
  instance.setLegend(plotOptions);
  const legendHeight = instance.legendType === "continuous" ? 50 : ((_d = plotOptions.color) == null ? void 0 : _d.label) ? 44 : 28;
  plotOptions.height = instance.hasLegend ? (plotOptions.height || 281) - legendHeight : plotOptions.height || 281;
  return plotOptions;
}
var src = {};
var sha3$1 = {};
...
const init$1 = ({
  ...
  return function cuid2() {
    ...
    const hashInput = `${time2 + salt + count2 + fingerprint}`;
    return `${firstLetter + hash(hashInput).substring(1, length2)}`;
  };
};
const createId$1 = init$1();
const isCuid$1 = (id2, { minLength = 2, maxLength = bigLength } = {}) => {
  const length2 = id2.length;
  const regex = /^[0-9a-z]+$/;
  try {
    if (typeof id2 === "string" && length2 >= minLength && length2 <= maxLength && regex.test(id2))
      return true;
  } finally {
  }
  return false;
...
function handleProperty(instance, prop, column2, options, propertyName) {
  const columnValue = column2 === false || column2 === null ? "" : column2;
  if (column2 !== void 0 && !equal2(columnValue, prop.column)) {
    if (!(propertyName === "color" && isColor(prop.column) && typeof column2 === "string" && isColor(column2))) {
      instance.newDataProps = true;
    }
  }
  if (column2 === false || column2 === null) {
    prop.column = "";
    prop.options = void 0;
  } else {
    if (column2 !== void 0) prop.column = column2;
    if (options !== void 0) prop.options = options;
  }
  return column2 === void 0 ? prop : instance;
}
function computeInterval(data, column2 = "x", isDate = true) {
  if (data.length === 0) return void 0;
  return isDate ? computeDateInterval(data, column2) : computeNumericInterval(data, column2);
}
function computeDateInterval(data, column2 = "x") {
  const sortedData = Array.from(
    new Set(data.map((d) => +d[column2]))
  ).sort((a2, b) => a2 - b);
  if (sortedData.length < 2) {
    return void 0;
  }
  const differences = pairs(sortedData).map(([a2, b]) => b - a2);
  const minDifference = min$2(differences) ?? 0;
  const second2 = 1e3;
  const minute = 60 * second2;
  ...
  if (minDifference < second2) {
    ...
    return utcYear.every(10);
  } else {
    return utcYear.every(100);
  }
}
function computeNumericInterval(data, column2 = "x") {
  const sorted = Array.from(
    new Set(data.map((d) => +d[column2]))
  ).sort((a2, b) => a2 - b);
  if (sorted.length < 2) return void 0;
  const differences = pairs(sorted).map(([a2, b]) => b - a2);
  return min$2(differences);
}
function getPrimaryMarkOptions(instance, markType) {
  var _a, _b, _c;
  if (!markType) return {};
  const { types } = instance.data();
  const data = instance.filteredData ?? instance.data();
  const currentColumns = Object.keys(data.types || {});
  const userOptions = instance.mark().options ?? ((_a = instance.markColumn().options) == null ? void 0 : _a[markType]);
  const color2 = isColor((_b = instance.color()) == null ? void 0 : _b.column) ? (_c = instance.color()) == null ? void 0 : _c.column : defaultColors[0];
  const stroke = currentColumns.includes("series") ? "series" : color2;
  const fill = currentColumns.includes("series") ? "series" : color2;
  const fx = currentColumns.includes("fx") ? "fx" : void 0;
  const sort2 = (userOptions == null ? void 0 : userOptions.sort) ?? ((types == null ? void 0 : types.x) !== "string" && markType !== "barX" && markType !== "rectX") ? { sort: (d) => d.x } : {};
  let interval2;
  const isRectYWithContinuousX = markType === "rectY" && ["date", "number"].includes((types == null ? void 0 : types.x) || "");
  const isRectXWithContinuousY = markType === "rectX" && ["date", "number"].includes((types == null ? void 0 : types.y) || "");
  if ((isRectYWithContinuousX || isRectXWithContinuousY) && (userOptions == null ? void 0 : userOptions.interval) === void 0) {
    const axis2 = markType === "rectY" ? "x" : "y";
    const isDate = (types == null ? void 0 : types[axis2]) === "date";
    interval2 = computeInterval(data, axis2, isDate);
  }
  return {
    ...
    ...markType === "text" && currentColumns.includes("text") ? { text: "text" } : {},
    ...fx ? { fx: `fx` } : {},
    ...currentColumns.includes("y") ? { y: `y` } : {},
    ...userOptions ? { ...userOptions } : {},
    ...interval2 ? { interval: interval2 } : {},
    ...currentColumns.includes("series") ? {
      [markType === "line" || (markType == null ? void 0 : markType.startsWith("rule")) || (markType == null ? void 0 : markType.startsWith("tick")) ? "stroke" : "fill"]: `series`
    } : {},
    ...instance.config().customRender ? { render: instance.config().customRender } : {}
  };
}
function getTipMarks(instance) {
  var _a, _b, _c, _d, _e, _f, _g, _h, _i;
  const plotOptions = instance.derivePlotOptions();
  const type2 = instance.mark().type;
  const xLabel = ((_a = instance.config().tipLabels) == null ? void 0 : _a.x) ?? ((_b = plotOptions.x) == null ? void 0 : _b.label) ?? "", yLabel = ((_c = instance.config().tipLabels) == null ? void 0 : _c.y) ?? ((_d = plotOptions.y) == null ? void 0 : _d.label) ?? "", xValue = (_e = instance.config().tipValues) == null ? void 0 : _e.x, yValue = (_f = instance.config().tipValues) == null ? void 0 : _f.y;
  const data = instance.filteredData ?? instance.data();
  const currentColumns = Object.keys(data.types || {});
  const fx = currentColumns.includes("fx") ? "fx" : void 0;
  let interval2;
  const userOptions = instance.mark().options ?? ((_g = instance.markColumn().options) == null ? void 0 : _g[type2]);
  const hasDateX = type2 === "rectY" && ((_h = data.types) == null ? void 0 : _h.x) === "date";
  const hasDateY = type2 === "rectX" && ((_i = data.types) == null ? void 0 : _i.y) === "date";
  if ((hasDateX || hasDateY) && (userOptions == null ? void 0 : userOptions.interval) === void 0) {
    interval2 = computeInterval(data, type2 === "rectY" ? "x" : "y");
  }
  const options = {
    ...
    channels: {
      xCustom: {
        ...
        value: typeof xValue === "function" ? xValue : currentColumns.includes("fx") ? "fx" : "x"
      },
      yCustom: {
    ...
    format: {
      ...
      fx: false,
      z: false
      // Hide the auto generated "series" for area charts
    },
    stroke: borderOptions.borderColor,
    ...currentColumns.includes("x") && !hasDateX ? { x: `x` } : {},
    ...currentColumns.includes("x") && hasDateX && interval2 ? { x1: `x`, x2: (d) => interval2.offset(d.x, 1) } : {},
    ...currentColumns.includes("fy") ? { fy: "fy" } : {},
    ...fx ? { fx: `fx` } : {},
    ...currentColumns.includes("y") ? { y: `y` } : {},
    ...currentColumns.includes("y") && hasDateY && interval2 ? { y1: `y`, y2: (d) => interval2.offset(d.y, 1) } : {},
    ...interval2 ? { interval: interval2 } : {},
    ...currentColumns.includes("series") ? {
      [type2 === "line" || (type2 == null ? void 0 : type2.startsWith("rule")) || (type2 == null ? void 0 : type2.startsWith("tick")) ? "stroke" : "fill"]: `series`
    } : {}
  };
  const maybeStackedOptions = type2 === "areaY" || type2 === "barY" || type2 === "rectY" ? stackY(options) : type2 === "barX" || type2 === "rectX" ? stackX(options) : options;
  const pointer2 = type2 === "barX" || type2 === "rectX" ? pointerY : pointerX;
  let marks2 = [tip(instance.filteredData, pointer2(maybeStackedOptions))];
  const otherMark = instance.config().tipMark;
  if (otherMark == null ? void 0 : otherMark.type) {
    let x2, y2;
    if (hasDateX && interval2)
      x2 = (d) => (+d.x + +interval2.offset(d.x, 1)) / 2;
    if (hasDateY && interval2)
      y2 = (d) => (+d.y + +interval2.offset(d.y, 1)) / 2;
    ...
    marks2.push(otherTip);
  }
  return marks2;
}
const ellipsis = "…";
function truncateLabel(label, length2 = 25) {
  if (!label || label.length < length2) return label;
  return label.slice(0, length2) + ellipsis;
}
function getTreemapMarks(data, instance) {
  var _a, _b, _c, _d, _e;
  const plotOptions = instance.derivePlotOptions();
  const yLabel = ((_a = instance.config().tipLabels) == null ? void 0 : _a.y) ?? ((_b = plotOptions.y) == null ? void 0 : _b.label) ?? "";
  const hideTip = instance.isServer || ((_c = instance.config()) == null ? void 0 : _c.tip) === false;
  const hasSeries = instance.color().column && !isColor(instance.color().column);
  const fill = isColor((_d = instance.color()) == null ? void 0 : _d.column) ? (_e = instance.color()) == null ? void 0 : _e.column : defaultColors[0];
  ...
  return [
    ...
    text(data, {
      ...
      dy: 8,
      text: (d) => {
        const v = d.data.text ? `${d.data.text}` : d.parent.data.name || `${d.value.toLocaleString()}`;
...
function prepareTreemapData(data, instance) {
  if (!data) return [];
  const { width, height } = instance.derivePlotOptions();
  const total = sum$1(data, (d) => d.y || 0);
  const groupedData = Array.from(
    group$1(data, (d) => d.series || d.text || false),
    ([key, values2]) => ({
      name: key,
      children: values2.map((v) => ({
        name: v.series,
        // TODO: think about this.... maybe rename to category
        ...v,
        percent: (v.y / total * 100).toFixed(1)
      }))
    })
  ...
  treemap().size([width ?? 500, height ?? 500]).padding(1)(root2);
  return root2.leaves();
}
function getCirclePackMarks(data, instance) {
  var _a, _b, _c, _d, _e;
  const plotOptions = instance.derivePlotOptions();
  const yLabel = ((_a = instance.config().tipLabels) == null ? void 0 : _a.y) ?? ((_b = plotOptions.y) == null ? void 0 : _b.label) ?? "";
  const textLabel = instance.text().column ?? "";
  ...
  return [
    // Parent circle
    dot(
      data.descendants().filter((d) => !d.parent),
      {
        x: "x",
        y: "y",
        r: "r",
    ...
    dot(
      data.descendants().filter(
        (d) => {
          var _a2, _b2, _c2, _d2;
          return ((_b2 = (_a2 = d.data) == null ? void 0 : _a2.children) == null ? void 0 : _b2.length) !== 1 && // has more than one child
          d.parent && // has a parent
          ((_d2 = (_c2 = d.parent) == null ? void 0 : _c2.children) == null ? void 0 : _d2.length) !== 1;
        }
        // if it's the only child, don't draw a circle
      ),
      {
        x: "x",
        y: "y",
...
class Arc extends Mark {
  constructor(data, options) {
    ...
    __publicField(this, "render", (index, scales, channels, dimensions, context) => {
      var _a, _b, _c, _d;
      if (this.instance.config().customRender) {
        this.instance.config().customRender.call(
          this,
          // ← bind Arc instance as `this`
          index,
          scales,
          channels,
          dimensions,
      ...
      const indexAccessor = (arr) => (i) => (arr == null ? void 0 : arr[i]) ?? 0;
      const arcGen = shapeArc().startAngle(indexAccessor(SA)).endAngle(indexAccessor(EA)).innerRadius(indexAccessor(RI)).outerRadius(indexAccessor(RO));
      const fillFn = scales.color ? (i) => {
        var _a2;
        return (_a2 = scales == null ? void 0 : scales.color) == null ? void 0 : _a2.call(scales, this.fill(this.data[i]));
      } : (i) => this.fill(this.data[i]);
      const doc = ((_a = this.instance) == null ? void 0 : _a.document) ?? (typeof document !== "undefined" ? document : null);
      if (!doc) return null;
      const gEl = doc.createElementNS("http://www.w3.org/2000/svg", "g");
      const g = select$1(gEl).attr("class", "arc");
      const { data, instance } = this;
      let lastActiveTooltipClass = null;
      for (let i = 0; i < this.data.length; ++i) {
        const series = (_b = this.data[i]) == null ? void 0 : _b.series;
        const sliceId = `${this.instance.id()}-${series}`;
        const className = toSafeClassName(sliceId);
        g.append("path").attr("d", arcGen(i)).attr("fill", fillFn(i)).attr("transform", `translate(${(_c = scales.x) == null ? void 0 : _c.call(scales, 0)},${(_d = scales.y) == null ? void 0 : _d.call(scales, 0)})`).on("mouseenter", function() {
          var _a2;
          (_a2 = this.parentNode) == null ? void 0 : _a2.appendChild(this);
          const tipMarks = document.getElementsByClassName(className);
          for (let i2 = 0; i2 < tipMarks.length; i2++) {
            tipMarks[i2].style.visibility = "visible";
          }
          if (lastActiveTooltipClass && lastActiveTooltipClass !== className) {
            ...
            for (let i2 = 0; i2 < previousTips.length; i2++) {
              previousTips[i2].style.visibility = "hidden";
            }
          }
          ...
          const tipMarks = document.getElementsByClassName(className);
          for (let i2 = 0; i2 < tipMarks.length; i2++) {
            tipMarks[i2].style.visibility = "hidden";
          }
          if (instance.plotObject) {
...
function arc(data, options) {
  return new Arc(data, options);
}
function getPieMarks(data, instance) {
  var _a, _b, _c;
  if (!data) return [];
  const plotOptions = instance.derivePlotOptions();
  const outerRadius = Math.min((plotOptions.width ?? 0) - 20, (plotOptions.height ?? 0) - 20) / 2;
  const donut = instance.config().donut;
  const innerRadius = donut ? outerRadius / 2 : 0;
  const total = data.reduce((sum2, d) => sum2 + d.y, 0);
  const pieGen = d3pie().value((d) => d.y).padAngle(5e-3).sort(null);
  const pieData = pieGen(data).map((d) => {
    var _a2, _b2;
    const angle = (d.startAngle + d.endAngle) / 2;
    const domainExtent = donut ? 90 + 45 : 90;
    const labelRadius = domainExtent;
    const x2 = Math.sin(angle) * labelRadius / 2;
    ...
    return {
      ...
      endAngle: d.endAngle,
      series: (_a2 = d.data) == null ? void 0 : _a2.series,
      y: (_b2 = d.data) == null ? void 0 : _b2.y,
      x: x2,
      yPos,
      arcLength,
      percent
  ...
  const hideTip = instance.isServer || ((_a = instance.config()) == null ? void 0 : _a.tip) === false;
  const yLabel = ((_b = instance.config().tipLabels) == null ? void 0 : _b.y) ?? ((_c = plotOptions.y) == null ? void 0 : _c.label) ?? "";
  const render2 = (index, scales, channels, dimensions, context, next) => {
    if (next) {
      ...
      if (ele) {
        ...
        for (const el of ele.children) {
          const className = toSafeClassName(`${instance.id()}-${series == null ? void 0 : series[i]}`);
          const htmlEl = el;
  ...
  const tipMark = hideTip ? null : tip(pieData, {
    fill: "series",
    x: (d) => d.x,
    y: (d) => d.yPos,
    ariaLabel: (d) => d.series,
  ...
  const labelMark = displayPercentages ? text(pieData, {
    x: (d) => d.x,
    y: (d) => d.yPos,
...
function getAllMarkOptions(instance) {
  var _a, _b, _c;
  ...
  const currentColumns = ((_a = instance.filteredData) == null ? void 0 : _a.types) ? Object.keys((_b = instance.filteredData) == null ? void 0 : _b.types) : [];
  const isValidTickChart = mark === "tickX" && currentColumns.includes("x") || mark === "tickY" && currentColumns.includes("y");
  const hasX2 = currentColumns.includes("x");
  const hasY2 = currentColumns.includes("y");
  const hasAggregate = instance.config().aggregate !== void 0 && instance.config().aggregate !== false;
  const hasColumnsOrAggregate = hasX2 && hasY2 || (hasX2 || hasY2) && hasAggregate;
  const isValidTreemap = mark === "treemap" && hasY2;
  const isValidCirclePack = mark === "circlePack" && hasY2;
  const isValidPie = mark === "pie" && hasY2;
  const markColumnMarks = Array.from(
    new Set(instance.filteredData.map((d) => d.markColumn).filter((d) => d))
  );
  const showPrimaryMark = (isValidTickChart || hasColumnsOrAggregate || isValidTreemap || isValidCirclePack || isValidPie) && (mark || markColumnMarks.length > 0);
  const marks2 = markColumnMarks.length > 0 && instance.markColumn().column !== void 0 ? markColumnMarks : [mark];
  const primaryMarks = showPrimaryMark ? [
    ...marks2.map((mark2) => {
      var _a2;
      const markData = (_a2 = instance.filteredData) == null ? void 0 : _a2.filter(
        (d) => {
          return markColumnMarks.length > 0 ? d.markColumn === mark2 : true;
        }
      );
      return mark2 === "treemap" ? getTreemapMarks(prepareTreemapData(markData, instance), instance) : mark2 === "circlePack" ? getCirclePackMarks(
        prepareCirclePackData(markData, instance),
        instance
      ) : mark2 === "pie" ? getPieMarks(markData, instance) : Plot[mark2](
        markData,
        getPrimaryMarkOptions(instance, mark2)
      );
    })
  ].flat() : [];
  const commonPlotMarks = [
    ...
    // or pie
    ...mark === "treemap" || mark === "circlePack" || mark === "pie" ? [] : getCommonMarks(currentColumns),
    ...instance.options().marks || []
  ];
  const fyMarks = mark === "treemap" || mark === "circlePack" ? [] : getfyMarks(instance.filteredData, currentColumns, plotOptions.fy);
  const hideTip = instance.isServer || ((_c = instance.config()) == null ? void 0 : _c.tip) === false || !showPrimaryMark || // These marks handle their own tips
...
const yNodeSelector = 'g[aria-label="y-axis tick label"][text-anchor="end"] text, g[aria-label="y-axis tick label"] g[text-anchor="end"] text';
const xNodeSelector = '[aria-label="x-axis tick label"] text, [aria-label="fx-axis tick label"] text, [aria-label="fx-axis tick label"] g text';
const yNodeRightSelector = yNodeSelector.replaceAll("end", "start");
function PlotAutoMargin(config, options, font) {
  const getWidth2 = function(element) {
  ...
  function getRotation(nodes, totalWidth) {
    if (!nodes) return 0;
    const maxWidth = Math.max(
      ...Array.from(nodes).map((node) => getWidth2(node))
    );
    const gap = 5;
    return maxWidth > totalWidth / nodes.length - gap ? -45 : 0;
  }
  function adjustVisibility(nodes, size) {
  ...
  let maxYWidth = 0, maxYRightWidth = 0;
  yNodes.forEach((node) => {
    const computedWidth = getWidth2(node);
  ...
  const tickRotate = rotateX ? getRotation(xNodes, (config.width || 0) - (maxYWidth + maxYRightWidth)) : 0;
  let maxHeight = 0;
  let maxWidthFromX = 0;
  xNodes.forEach((node) => {
    const count2 = (node.innerHTML.match(/<tspan/g) || [""]).length;
    const height = 14 * count2;
    const computedWidth = getWidth2(node);
    const theta = tickRotate * Math.PI / 180;
    ...
    maxWidthFromX = tickRotate === 0 ? 0 : Math.max(maxWidthFromX, rotatedWidth);
  });
  let style = config.style || {};
  let facet = "fx" in config ? {
  ...
  if (hideWarnings) {
    ...
    const warning = Array.from(elements).find(
      (element) => {
        var _a;
        return (_a = element.textContent) == null ? void 0 : _a.includes("Please check the console");
      }
    );
...
async function legendCategorical(instance) {
  var _a, _b, _c, _d, _e, _f;
  const document2 = instance.document;
  const categories = (_c = Array.from(
    ((_b = (_a = instance.plotObject) == null ? void 0 : _a.scale("color")) == null ? void 0 : _b.domain) ?? []
  )) == null ? void 0 : _c.map((d) => `${d}`);
  const symbols2 = categories.map((category) => {
    var _a2;
    if (!instance.markColumn().column) return instance.mark().type;
    const data = instance.data();
    const symbol2 = (_a2 = data.find((d) => d.series == category)) == null ? void 0 : _a2.markColumn;
    return symbol2;
  });
  function isActive(category) {
    return instance.visibleSeries.length === 0 || instance.visibleSeries.includes(category);
  }
  const colors2 = Array.from(((_e = (_d = instance.plotObject) == null ? void 0 : _d.scale("color")) == null ? void 0 : _e.range) ?? []);
  const options = await instance.derivePlotOptions();
  const width = options.width || 500;
  const height = options.height || 300;
  const label = (_f = options.color) == null ? void 0 : _f.label;
  const font = instance.font;
  const hiddenContainer = document2.createElement("div");
  hiddenContainer.style.position = "absolute";
  hiddenContainer.style.visibility = "hidden";
  hiddenContainer.style.height = "0";
  hiddenContainer.style.overflow = "hidden";
  document2.body.appendChild(hiddenContainer);
  const legend2 = document2.createElement("div");
  const container = document2.createElement("div");
  container.style.maxWidth = `${width}px`;
  container.className = "dp-categories-container";
  if (label) {
    const legendLabel = document2.createElement("div");
    legendLabel.className = "dp-legend-label";
    legendLabel.innerHTML = label || "";
    legend2.appendChild(legendLabel);
  }
  const categoriesDiv = document2.createElement("div");
  categoriesDiv.className = "dp-categories";
  categories.forEach((category, i) => {
    const categoryDiv = document2.createElement("div");
    categoryDiv.className = `dp-category${isActive(category) ? "" : " dp-inactive"}`;
    const symbolType = symbols2[i];
    const symbol2 = drawSymbol(symbolType, colors2[i % colors2.length], document2);
    categoryDiv.appendChild(symbol2);
    const textNode = document2.createTextNode(category);
    categoryDiv.appendChild(textNode);
    categoryDiv.setAttribute("data-tooltip", category);
    categoriesDiv.appendChild(categoryDiv);
  });
  const collapsedCategoriesDiv = document2.createElement("div");
  collapsedCategoriesDiv.className = "dp-collapsed-categories";
  const popoverDiv = document2.createElement("div");
  ...
  legend2.appendChild(container);
  if (instance.config().interactiveLegend !== false && !instance.isServer) {
    collapsedCategoriesDiv.addEventListener(
      "click",
      () => showPopover(container, height)
    );
    const legendElements = legend2.querySelectorAll(".dp-category");
    legendElements.forEach((element) => {
      const elementId = `${element.textContent}`;
      if (!elementId) return;
      ...
      element.addEventListener("click", (event) => {
        ...
        if (mouseEvent.shiftKey) {
          if (instance.visibleSeries.length === 1 && isActive(elementId)) {
            ...
            instance.visibleSeries = [elementId];
          }
        } else {
          if (isActive(elementId)) {
            const currentSeries = instance.visibleSeries.length ? instance.visibleSeries : categories;
            instance.visibleSeries = currentSeries.filter(
              (id2) => id2 !== elementId
            );
          } else {
            instance.visibleSeries.push(elementId);
          }
        }
...
const getWidth = function(element, font) {
  if (font && font.getAdvanceWidth) {
...
function updateLegendDisplay(container, font) {
  ...
  if (visibleCount < totalCategories) {
    ...
    if (collapsedWidth + categoriesWidth > containerWidth) {
      ...
      collapsedCategories.textContent = `+${totalCategories - visibleCount + 1} more`;
      popover.appendChild(lastVisibleCategory);
    }
  } else {
    collapsedCategories.style.display = "none";
  }
}
function showPopover(container, height) {
  const popover = container.querySelector(".dp-popover");
...
function drawSymbol(symbolType, color2, document2) {
  const symbol2 = document2.createElement("div");
  symbol2.style.backgroundColor = color2;
  symbol2.style.width = "12px";
  switch (symbolType) {
    case "dot":
      symbol2.style.height = "12px";
      symbol2.style.borderRadius = "12px";
      symbol2.style.border = "1px solid rgba(0,0,0, .16)";
      return symbol2;
    case "line":
      symbol2.style.height = "0px";
      symbol2.style.borderTop = "2px solid rgba(0,0,0, .16)";
      return symbol2;
    default:
      symbol2.style.height = "12px";
      symbol2.style.borderRadius = "2px";
      symbol2.style.border = "1px solid rgba(0,0,0, .16)";
      return symbol2;
  }
}
async function legendContinuous(instance) {
  var _a, _b, _c;
  const color2 = (_a = instance.plotObject) == null ? void 0 : _a.scale("color");
  const document2 = instance.document;
  const onBrush = instance.config().interactiveLegend === false || instance.isServer ? null : (event) => {
  ...
  const label = ((_b = options.color) == null ? void 0 : _b.label) ?? ((_c = instance.data().labels) == null ? void 0 : _c.series);
  const plotLegend = legend({
    color: color2,
    label,
    width,
  ...
  if (onBrush !== null) {
    let brushed = function(event) {
      ...
      if (event.selection) {
        const [x02, x12] = event.selection;
        if (onBrush) onBrush([scale2.invert(x02), scale2.invert(x12)]);
      } else {
        if (onBrush) onBrush([]);
      }
    };
    const height = 50;
    const svg = select$1(container).append("svg").attr("width", width).attr("height", height).style("position", "absolute").style("top", "0px").style("left", "0px");
    const brush2 = brushX().extent([
      [0, 0],
      [width, height]
    ]).on("brush end", brushed);
    const brushGroup = svg.append("g").call(brush2);
    let isProgrammatic = false;
    const scale2 = linear().domain((color2 == null ? void 0 : color2.domain) ?? []).range([0, width]);
    if (instance.seriesDomain && instance.seriesDomain.length === 2) {
      const [x02, x12] = instance.seriesDomain.map(scale2);
      isProgrammatic = true;
      brushGroup.call(brush2.move, [x02, x12]);
    }
  }
  return container;
}
async function render(instance, newLegend) {
  var _a, _b;
  instance.setSorts();
  ...
  const autoMargin = instance.isServer && !instance.font ? false : instance.config().autoMargin !== false;
  instance.plotObject = autoMargin ? PlotAutoMargin(plotOptions, {}, instance.font) : plot(plotOptions);
  if (instance.config().onClick && !instance.isServer) {
    instance.plotObject.addEventListener(
      ...
      (event) => {
        ...
        instance.config().onClick(event, { ...(_a2 = instance.plotObject) == null ? void 0 : _a2.value });
        const pointerleave = new PointerEvent("pointerleave", {
          bubbles: true,
          pointerType: "mouse"
        });
        (_b2 = event.target) == null ? void 0 : _b2.dispatchEvent(pointerleave);
      },
      { capture: true }
    );
  }
  ...
  const allMarks = ((_a = instance.mark()) == null ? void 0 : _a.type) ? [instance.mark().type] : Array.from(
    new Set(
      ((_b = instance.filteredData) == null ? void 0 : _b.map((d) => d.markColumn).filter((d) => d)) || []
    )
  );
  if (allMarks.some((mark) => ["rectY", "rectX"].includes(mark))) {
    ...
    rects.forEach((rect2) => {
      if (allMarks.includes("rectY")) {
        const width = rect2.getAttribute("width");
        if (width && +width < minSize) {
          rect2.setAttribute("width", `${minSize}`);
        }
      }
      if (allMarks.includes("rectX")) {
        const height = rect2.getAttribute("height");
        if (height && +height < minSize) {
          rect2.setAttribute("height", `${minSize}`);
        }
      }
    });
  }
  const container = instance.chartContainer || instance.document.createElement("div");
  ...
  if (instance.hasLegend && newLegend) {
    const legendContainer = container.querySelector(".legend-container") || container.appendChild(instance.document.createElement("div"));
    legendContainer.className = "legend-container";
    legendContainer.innerHTML = "";
    const legend2 = instance.legendType === "categorical" ? await legendCategorical(instance) : await legendContinuous(instance);
...
async function renderError(instance, error) {
  const document2 = instance.isServer ? instance.jsdom.window.document : window.document;
  const errorElement = document2.createElement("div");
  errorElement.textContent = `Error rendering plot: ${error instanceof Error ? error.message : String(error)}`;
  return errorElement;
}
const emptyProp = { column: "", options: {} };
class DuckPlot {
  constructor(ddb, { jsdom, font } = {}) {
    __publicField(this, "_ddb", null);
    __publicField(this, "_table", null);
    __publicField(this, "_catalog", null);
    __publicField(this, "_x", { ...emptyProp });
    __publicField(this, "_y", { ...emptyProp });
    __publicField(this, "_fy", { ...emptyProp });
    __publicField(this, "_fx", { ...emptyProp });
    __publicField(this, "_r", { ...emptyProp });
    __publicField(this, "_text", { column: "" });
    __publicField(this, "_color", { ...emptyProp });
    __publicField(this, "_mark", {});
    __publicField(this, "_options", {});
    __publicField(this, "_jsdom");
    __publicField(this, "_font");
    __publicField(this, "_isServer");
    __publicField(this, "_document");
    __publicField(this, "_newDataProps", true);
    __publicField(this, "_data", []);
    __publicField(this, "_rawData");
    __publicField(this, "_markColumn", {});
    ...
    __publicField(this, "_sorts", {});
    __publicField(this, "_hasLegend");
    __publicField(this, "_legendType");
    // Rather than provide getter/setters, just make these public
    __publicField(this, "plotObject");
    __publicField(this, "visibleSeries", []);
    __publicField(this, "filteredData");
    __publicField(this, "chartContainer", null);
    __publicField(this, "seriesDomain", []);
    this._ddb = ddb;
    this._jsdom = jsdom;
    this._font = font;
    this._isServer = jsdom !== void 0;
    this._document = this._isServer ? this._jsdom.window.document : window == null ? void 0 : window.document;
    this._id = createId_1();
  }
  table(table) {
    if (table) {
  ...
  x(column2, options) {
    return handleProperty(this, this._x, column2, options);
  }
  y(column2, options) {
    return handleProperty(this, this._y, column2, options);
  }
  color(column2, options) {
    return handleProperty(this, this._color, column2, options, "color");
  }
  fy(column2, options) {
    return handleProperty(this, this._fy, column2, options);
  }
  fx(column2, options) {
    return handleProperty(this, this._fx, column2, options);
  }
  r(column2, options) {
    return handleProperty(this, this._r, column2, options);
  }
  text(column2) {
    return handleProperty(this, this._text, column2);
  }
  mark(type2, options) {
    if (type2) {
      if (this._mark.type !== type2) {
        this._newDataProps = true;
      }
      this._mark = { type: type2, ...options ? { options } : {} };
      return this;
    }
    return this._mark;
  }
  ...
  config(config) {
    if (config) {
      if (this._config.aggregate !== config.aggregate || this._config.percent !== config.percent) {
        this._newDataProps = true;
      }
      this._config = config;
      return this;
  ...
  markColumn(column2, options) {
    if (column2) {
      if (this._markColumn.column !== column2) {
        this._newDataProps = true;
      }
      this._markColumn = { column: column2, ...options ? { options } : {} };
      return this;
    }
  ...
  setSorts() {
    ...
    if (Object.keys(((_a = this._data) == null ? void 0 : _a.types) ?? {}).includes("fy")) {
      this._sorts = {
        ...this._sorts,
  ...
  setLegend(plotOptions) {
    var _a, _b, _c, _d;
    this._hasLegend = ((_a = this._data.types) == null ? void 0 : _a.series) !== void 0 && ((_b = plotOptions.color) == null ? void 0 : _b.legend) !== null && ((_c = plotOptions.color) == null ? void 0 : _c.legend) !== false;
    this._legendType = ((_d = plotOptions.color) == null ? void 0 : _d.type) ?? getLegendType(this._data);
  }
  // Render the plot (calls prepareData)
  async render(newLegend = true) {
    try {
