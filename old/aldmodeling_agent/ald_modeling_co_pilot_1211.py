...
# ============================================================
# Equation Extraction Agent (Vision-based, numbered equations)
# ============================================================
...
def vision_equation_extractor_agent(
    ...
    max_equations: int = 5,
    save_output: bool = True,
    output_dir: str = "./outputs_1211",
) -> List[Dict[str, Any]]:
    """
    Extract numbered display-style equations from a PDF using a Vision LLM.

    Canonical variable symbols from `variable_list` are provided as
    reference hints only. Equations must not be rewritten or normalized.
    ...
    pdf_path : str
        Path to the PDF file.
    vector_store : SimpleVectorStore
        Vector store to save extracted equations.
    variable_list : List[Dict[str, Any]]
        Output from the variable extraction agent.
    model : str
        Vision-capable OpenAI model.
    max_equations : int
        Maximum number of equations to keep (sorted by equation number).
    save_output : bool
        Whether to save equations.json.
    output_dir : str
    ...
    # --------------------------------------------------
    # Build variable reference (canonical symbols only)
    # --------------------------------------------------
    canonical_symbols = sorted(
        {
            v.get("canonical_symbol")
            for v in variable_list
            if v.get("canonical_symbol")
        }
    )

    variable_reference_block = "\n".join(
        f"- {sym}" for sym in canonical_symbols
    )
...
1. Find ONLY display-style equations that have an explicit equation number
   such as (1), (8), or (23), usually printed near the equation.
   - Ignore inline math.
   - Ignore textual references like "Eq. (1)" inside paragraphs.

2. For each numbered equation, extract:
   - number_str: the equation number exactly as printed (e.g., "(1)")
   - eq_number: the integer value of the number (e.g., 1)
   - latex: a clean LaTeX representation of the equation body
            (do NOT include the equation number).

3. If no numbered equations appear on the page, return an empty list.
...
Constraints:
- Only block (display-style) equations.
- The equation must have a visible number like (n).
- Ignore inline math and textual references.

REFERENCE VARIABLES (canonical namespace, for recognition only):
{variable_reference_block}

Rules regarding reference variables:
- Use this list ONLY as a hint for symbol recognition.
- Do NOT rewrite, normalize, or modify equations.
- Output LaTeX must exactly match what is visible in the image.

Return STRICT JSON as described.
"""

    # --------------------------------------------------
    # Page-wise Vision calls
    # --------------------------------------------------
    ...
    if save_output:
        ...
        print(f"[Vision Equation Extractor] Saved: {out_path}")
...
#  with Composite Variable Inference
# ===========================================================
...
class AgentLoggerMixin:
    def log(self, agent_name: str, filename: str, data: Any):
        ...
        safe_name = str(filename) if filename is not None else "unknown"
        fp = out / f"{agent_name}__{safe_name}.json"
...
- Use the nomenclature as the authoritative reference
...
"""
    ...
    # --------------------------------------------------
    # Composite inference logic (NEW)
    # --------------------------------------------------
    def infer_composite_variables(
        self,
        latex: str,
        atomic_vars: List[Dict[str, Any]],
        nomenclature: Dict[str, Any],
    ) -> List[Dict[str, Any]]:
        """
        Infer composite canonical variables from atomic mappings.

        Example:
        Theta + P^C_{sat}  -->  g_pc[sat]
        """

        canonicals = {v["canonical"] for v in atomic_vars if v.get("canonical")}

        inferred: List[Dict[str, Any]] = []

        for canonical_name in nomenclature.keys():

            # Only consider composite-like canonical variables
            if "[" not in canonical_name:
                continue

            # Heuristic signals
            has_sat = "sat" in canonical_name.lower()
            has_pc = "pc" in canonical_name.lower()

            if not (has_sat and has_pc):
                continue

            # Evidence from atomic vars and latex
            evidence = []

            if "theta" in canonicals:
                evidence.append("theta")

            if re.search(r"P\s*\^\s*C|PC", latex):
                evidence.append("PC")

            if re.search(r"_\{?sat\}?|sat", latex, re.IGNORECASE):
                evidence.append("sat")

            # Threshold: require at least 2 independent signals
            if len(evidence) >= 2:
                inferred.append(
                    {
                        "symbol_raw": " ".join(evidence),
    ...
    def run(self, ctx: Dict[str, Any]) -> Dict[str, Any]:
        ...
        output_root = Path(ctx.get("output_root", "./outputs"))
        ...
        for pdf_name, eq_list in eqs_by_pdf.items():
            ...
            pdf_out_dir = output_root / pdf_name
            pdf_out_dir.mkdir(parents=True, exist_ok=True)

            for idx, eq in enumerate(eq_list):

                latex = eq.get("latex", "")
                eq_number = eq.get("eq_number", idx)
                eq_id = f"{pdf_name}_eq{eq_number}"

                # --------------------------------------------------
                # Phase 1: Atomic variable mapping (LLM)
                # --------------------------------------------------
                ...
                atomic_vars = data.get("variables", [])

                # --------------------------------------------------
                # Phase 2: Composite inference (NEW)
                # --------------------------------------------------
                composite_vars = self.infer_composite_variables(
                    latex=latex,
                    atomic_vars=atomic_vars,
                    nomenclature=nomenclature_dict,
                )

                # Remove atomic components if composite detected
                if composite_vars:
                    suppressed = {
                        v["canonical"]
                        for v in atomic_vars
                        if v.get("canonical") in {"theta"}
                    }
                    atomic_vars = [
                        v for v in atomic_vars
                        if v.get("canonical") not in suppressed
                    ]

                final_vars = atomic_vars + composite_vars

                result_entry = {
                    "eq_id": eq_id,
                    "eq_number": eq_number,
                    "latex": latex,
                    "page": eq.get("page_index"),
...
"""
    ...
    def _llm_extract(self, sentence: str, canonical_list: List[str]) -> Dict[str, Any]:
    ...
    def run(self, ctx: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for pdf_name, sentences in sentences_by_pdf.items():
            ...
            nomenclature = variables_by_pdf.get(pdf_name, {})
            canonical_list = list(nomenclature.keys())
            ...
            for sent in sentences:
                ...
                if (
                    result["variable_mentions"]
                    or result["variable_definitions"]
            ...
            # --------------------------------------------------
            # Save results to output_root/<pdf_name>/text_knowledge.json
            # --------------------------------------------------
...
def run_full_pipeline(
    ...
    max_equations_per_pdf: int = 20,
    language: str = "en",
):
    """
    End-to-end execution of the Equation & Variable Co-Pilot pipeline.

    Flow:
      PDF
        → nomenclature (vision)
        → variables.json
        → equations.json (vision)
        → equation-variable mapping
    ...
    print("\n==============================")
    print(" Equation & Variable Co-Pilot ")
    print("==============================\n")
    ...
    # --------------------------------------------------
    # 1) Load RAG documents (optional background knowledge)
    # --------------------------------------------------
    try:
        rag_docs = load_rag_documents()
        vector_store.add_documents(rag_docs)
        print(f"[INIT] Loaded {len(rag_docs)} RAG documents")
    except Exception as e:
        print(f"[WARN] RAG loading failed: {e}")

    # --------------------------------------------------
    # 2) PDF → raw text
    # --------------------------------------------------
    pdf_text_agent = PDFTextExtractionAgent(pdf_dir=document_dir)
    ctx = pdf_text_agent.run(ctx)

    # --------------------------------------------------
    # 3) Vision-based variable extraction (nomenclature)
    # --------------------------------------------------
    ctx["variables_nomenclature"] = {}
    ctx["variables_raw"] = {}

    for pdf_path in Path(document_dir).glob("*.pdf"):
        pdf_name = pdf_path.name

        # convention: nomenclature image next to PDF
        nomenclature_img = pdf_path.with_suffix("").as_posix() + "_nomenclature.png"

        if not os.path.exists(nomenclature_img):
            print(f"[SKIP] No nomenclature image for {pdf_name}")
            ctx["variables_nomenclature"][pdf_name] = {}
            continue

        print(f"[VISION] Extracting variables from {nomenclature_img}")

        variables = vision_variable_extractor_agent(
            nomenclature_image_path=nomenclature_img,
        ...
        # raw → canonical dictionary
        raw2canon = {
            v["symbol_raw"]: v.get("canonical_symbol")
            for v in variables
        }

        ctx["variables_nomenclature"][pdf_name] = raw2canon
        ctx["variables_raw"][pdf_name] = variables

    # --------------------------------------------------
    # 4) Vision-based equation extraction
    # --------------------------------------------------
    ctx["equations"] = {}

    for pdf_path in Path(document_dir).glob("*.pdf"):
        pdf_name = pdf_path.name
        var_list = ctx["variables_raw"].get(pdf_name, [])

        print(f"[VISION] Extracting equations from {pdf_name}")

        equations = vision_equation_extractor_agent(
            pdf_path=str(pdf_path),
            vector_store=vector_store,
            variable_list=var_list,
            model=vision_model,
            max_equations=max_equations_per_pdf,
            save_output=True,
            output_dir=output_root,
        )

        ctx["equations"][pdf_name] = equations

    # --------------------------------------------------
    # 5) Equation → Variable Mapping
    # --------------------------------------------------
    eq_var_mapper = EquationVariableMappingAgent(model=text_model)
    ctx = eq_var_mapper.run(ctx)

    # --------------------------------------------------
    # 6) Text sentence extraction
    # --------------------------------------------------
    sentence_agent = TextSentenceExtractionAgent()
    ctx = sentence_agent.run(ctx)

    # --------------------------------------------------
    # 7) Text knowledge extraction
    # --------------------------------------------------
    text_knowledge_agent = TextKnowledgeExtractionAgent(model=text_model)
    ctx = text_knowledge_agent.run(ctx)

    # --------------------------------------------------
    # 8) Relation reasoning
    # --------------------------------------------------
    relation_agent = RelationReasoningAgent()
    ctx = relation_agent.run(ctx)

    # --------------------------------------------------
    # 9) Knowledge graph construction
    # --------------------------------------------------
    kg_builder = KnowledgeGraphBuilderAgent()
    ctx = kg_builder.run(ctx)

    # --------------------------------------------------
    # 10) Knowledge graph summarization
    # --------------------------------------------------
    kg_summary_agent = KnowledgeGraphSummaryAgent(model=text_model)
    ctx = kg_summary_agent.run(ctx)
    ...
    ctx = interpretation_agent.run(ctx)

    # --------------------------------------------------
    # 12) Optimization interpretation
    # --------------------------------------------------
    ...
    # --------------------------------------------------
    # 13) Optimization relation extraction
    # --------------------------------------------------
    opt_rel_agent = OptimizationRelationExtractionAgent(model=text_model)
    ctx = opt_rel_agent.run(ctx)

    # --------------------------------------------------
    # 14) Optimization knowledge graph
    # --------------------------------------------------
    opt_kg_builder = OptimizationKGBuilder()
    ctx = opt_kg_builder.run(ctx)
