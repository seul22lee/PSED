...
if not EMAIL:
    sys.exit("Set $UNPAYWALL_EMAIL (or $CROSSREF_EMAIL, or config/email.txt) for the "
             "Crossref polite pool.")
UA = {"User-Agent": f"PSED-corpus/0.1 (https://github.com/psed; mailto:{EMAIL})"}
...
def resolve_bibliographic(ref):
    ...
    try:
        r = SESSION.get("https://api.crossref.org/works",
                        params={"query.bibliographic": q, "rows": 1, "mailto": EMAIL},
                        timeout=30)
