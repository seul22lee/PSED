...
Usage:
    python3 scripts/kb_migration_diff.py \
        --before <a previous resolved/ snapshot> \
...
def main(argv=None):
    ...
    ap.add_argument("--before", required=True,
                    help="a previous resolved/ snapshot to diff against")
    ...
    for doi in S.papers():
        after_p = P.resolved_json(doi, "experiments")
        if not after_p.exists():
            continue
        before = load(before_root / doi / "experiments.json")
        after = load(after_p)
        series = load(P.resolved_json(doi, "series"))
    ...
    summary = {
        "generator": "scripts/kb_migration_diff.py",
