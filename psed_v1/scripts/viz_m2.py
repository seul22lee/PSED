...
The canonical M2 report is produced by m2_design.py at twin/m2_report.html.
...
so the canonical report (or an ad-hoc review) can reuse them. Two panels:
  ...
  1. Recipe grounding — every experiment lifted to a Recipe, gaps filled from the
     ...
     fields came from covariate-conditioned KB inference vs model defaults.  (see the M2 report)t/recipes.json)
...
def main():
    ...
    twin/m2_report.html. This entry point no longer writes an artifact, so two
    ...
    print("viz_m2 is legacy and non-canonical — it writes no artifact.\n"
          "The canonical M2 report is m2_design.py -> twin/m2_report.html.")
    return
