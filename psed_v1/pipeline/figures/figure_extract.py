...
def _load_key():
    for line in (P.RESOURCES / "config" / "corpus" / ".env").read_text().splitlines():
...
def caption_fig_index(paper_id):
    """docling crop INDEX (as string) -> crop record, from the FIGURE INVENTORY.

    Reads the inventory rather than structure.json so that a crop whose caption
    Docling failed to bind still arrives here with its recovered caption, its printed
    figure number and its sibling set. Keyed by docling index — the machine routing
    id — never by printed figure number.
    """
    inv = _inventory.load(paper_id)
    out = {}
    for c in inv["candidates"]:
        if not c.get("image"):
            continue
        out[str(c["docling_index"])] = {
            "index": c["docling_index"],
            "image": c["image"],
            "caption": _inventory.caption_for(c),
            "caption_source": c.get("caption_source"),
            "printed_figure": c.get("printed_figure"),
            "siblings": c.get("siblings") or [],
            "printed_group_id": c.get("printed_group_id"),
        }
    return out


def _upstream_fingerprint(d):
    """Identity of the scout result this stage's output is derived from."""
    import hashlib
    p = d / "scout.json"
...
def _write_json(path, obj):
    """Write via a temp file + replace so a crash cannot leave a half-written artifact
...
def extract_paper(sd, client):
    ...
    fp = _upstream_fingerprint(d)
    if not scout.get("drill"):
        # An empty drill used to return HERE, before the writes below — so a paper that
        # newly selects nothing kept whatever figure_data.json/records.json an older,
        # incompatible run had left behind (proven on cnma.201700148, which carried
        # vision token counts that an empty drill cannot produce). Now the empty result
        # is itself written, which invalidates the stale artifacts instead of preserving
        # them. Writes are atomic and happen only after the scout result was read
        # successfully, so a valid old output is never destroyed by a failed run.
        print(f"[05 skip] {sd}: drill=0 — nothing to read (go_deeper={scout.get('go_deeper')})")
        _write_json(d / "figure_data.json", {"doi": sd, "process_card": {}, "figures": [],
                                             "_tokens": {"in": 0, "out": 0},
                                             "_upstream_scout": fp,
                                             "_note": "scout selected no figures"})
        _write_json(d / "records.json", [])
        return [], [], 0, 0
    fig_by_num = caption_fig_index(sd)
    ...
    for image, g in groups.items():
        ...
        if g.get("split"):
            # The caption describes the WHOLE printed figure, but this image is only one
            # crop of it. Demanding the caption's panel count here is what made the model
            # invent a second panel for 6.0002436 FIG. 1: it split one plot's two y-series
            # into "panel a" and "panel b" to satisfy a caption naming (a) and (b).
            hint = ("This image is ONE CROP of printed figure %s, which was split across "
                    "several images. The caption below describes the WHOLE printed figure, "
                    "so panels it names may NOT be in this image at all. Return ONLY the "
                    "panels you can actually SEE in this image. If the image is a single "
                    "plot, return exactly ONE panel — do NOT split its curves into "
                    "separate panels to match the caption, and do NOT invent a panel that "
                    "is not visible. Data of type(s): %s."
                    % (g.get("printed_figure") or "?", ", ".join(data_types)))
        ...
        if g.get("split"):
            # No expectation may be derived from a caption that describes other crops too.
            # Retrying "until the panel count matches the caption" against a single-plot
            # crop is precisely the pressure that fabricates panels.
            expected = 0
        else:
        ...
        results.append({"figure": g["fig"], "image": image, "caption": g["caption"],
                        "source": g["source"], "panel_source": g["panel_source"],
                        # printed identity travels alongside the machine index, never
                        # replacing it: g["fig"] stays the docling routing index.
                        "printed_figure": g.get("printed_figure"),
                        "is_split_crop": bool(g.get("split")),
                        "caption_source": g.get("caption_source"), **obj})

    _write_json(d / "figure_data.json",
                {"doi": sd, "process_card": card, "figures": results,
                 "_tokens": {"in": tok_in, "out": tok_out}, "_upstream_scout": fp})
...
def flatten_records(sd, scout, figresults):
    ...
    for fr in figresults:
        # citable printed number: from the inventory when provenance recovery resolved it,
        # else parsed from the caption. Kept strictly separate from the docling index.
        paper_fig = fr.get("printed_figure") or _cap_fignum(fr.get("caption"))
