...
extracted/{sd}/pressure.json.
  ...
  python3 -m pipeline.text.pressure <doi> [<doi> ...]   # extract (LLM)
  python3 -m pipeline.text.pressure --show <doi> ...     # print cached pressure.json
...
def extract_pressures(sd, client):
    ...
    st = json.loads((P.extracted_dir(sd) / "structure.json").read_text()) \
        if (P.extracted_dir(sd) / "structure.json").exists() else {}
    ...
    (P.extracted_dir(sd) / "pressure.json").write_text(
...
def pressure_facts(sd, reactants=None):
    ...
    pf = P.extracted_dir(sd) / "pressure.json"
    if not pf.exists():
        return []
    try:
