...
with `geometry` (structure) + `geometry_class` so geometry-scoped model validation works.
  ...
  python3 -m pipeline.text.geometry            # extract + tag all papers in the KB
  python3 -m pipeline.text.geometry --tag-only # re-tag experiments from cached geometry.json (no LLM)
"""
import paths as P
...
from ontology import vocab as _lib
