#!/usr/bin/env python3
"""
pipeline/parse/docling_parse.py — stage 1: PDF -> document structure. NO LLM.

    python3 cli.py parse <pdf> [<pdf> ...]

Writes, for each PDF, into the ONE per-paper root:

    papers/<paper_id>/extracted/document.md      full markdown
    papers/<paper_id>/extracted/structure.json   {n_pages, sections[headings],
                                                  figures[{index,caption,image}],
                                                  tables[{index,caption,markdown}]}
    papers/<paper_id>/extracted/figures/fig_N.png

This stage once wrote to a directory no other stage read
after the paper-folder migration -- the parse output was silently orphaned.
It now resolves every path through `paths.py`, like every other stage.
"""
import paths as P
import json, re, sys
...
def main(pdfs):
    for pdf in pdfs:
        ...
        print(f"  {len(md)} md chars · {struct['n_figures']} figures · {struct['n_tables']} tables "
              ...
              f"{' (OCR)' if struct.get('ocr_forced') else ''} → papers/{sd}/extracted/")
