...
def _load_canonical():
    """Canonical curves produced by pipeline/canonical/build_canonical.py."""
    out = []
    for d in sorted(P.glob_canonical("curves.json")):
        try:
...
def _load_entities():
    ...
    out = []
    for d in sorted(P.glob_resolved("entities.json")):
        try:
...
def main():
    exps = []
    series_recs = []
    for d in sorted(P.glob_resolved("experiments.json")):
    ...
    # produced months ago, and every consumer of it -- ontology/validate.py,
    ...
    (P.knowledge_graph_json()).write_text(json.dumps(graph, indent=1))
