#!/usr/bin/env python3
"""
supplements.py — recover source evidence the extraction stage lost.

Two independent loss classes are known in the corpus, and both are handled by ONE generic
...
It is emitted as a Measurement carrying its caption evidence and NO ResultSeries,
...
WHY it was needed, so the two causes stay distinguishable:
  ...
  · caption grammar — the panel marker was written '( a )' with internal spaces, which
    ...
    reached the vision stage. This layer's copy of the parser accepts the spaced form.
...
Local and deterministic throughout: this layer's copy of the caption parser, and PyMuPDF
...
def missing_panels(paper):
    ...
    `paper` is a build_semantic.Paper. Returns a list of supplement records.
    """
    covered = set()
...
_SYNTH_CLAUSE = re.compile(
    r"\b(?:deposited|grown|coated|prepared|synthesi[sz]ed|covered\s+by|capped\s+with)\b",
    re.I)
_STRUCTURE_CLAUSE = re.compile(
    r"\d+(?:\.\d+)?\s*(?:nm|µm|um|μm|Å)\b|\b(?:single\s+layer|bilayer|stack|"
...
def image_supported_cases(paper, note=None):
    ...
    for pf in sorted((set(paper._printed.values()) | from_doc) - covered,
        ...
        # A caption may identify the deposited object by DESCRIBING it rather than by
        # restating process conditions -- "an ALD SiO2 film of 7.0 nm thickness deposited
        # on a H-terminated Si(100) wafer, covered by an Al2O3 film". That is a synthesis
        # statement local to this figure and identifies the deposition just as well.
        structural = bool(_STRUCTURE_CLAUSE.search(cap)) and _SYNTH_CLAUSE.search(cap)
        if not deposited or not (conds or structural):
            continue
