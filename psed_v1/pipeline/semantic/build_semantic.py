#!/usr/bin/env python3
"""
build_semantic.py — build the semantic layer for one paper.

The production semantic stage: reads papers/<pid>/{extracted,resolved,canonical}
through the paths API and writes the semantic JSON files under
papers/<pid>/semantic/. The scientific behaviour is the one validated in the
semantic pilot and Workbench v2; this module is its production home.

    python3 cli.py semantic --paper <doi>        one paper
    python3 cli.py semantic --all                every paper in the corpus manifest
...
from pipeline.semantic import cases as PC
from pipeline.semantic import evidence as PE
from pipeline.semantic import roles as R
from pipeline.semantic import sample_table as PT
from pipeline.semantic import design as D
from pipeline.semantic import supplements as SUP
from pipeline.semantic import ranges as PRG
...
class Paper(object):
    def __init__(self, pid):
        self.pid = pid
        self.root = P.paper_dir(pid)
        self.ex = P.extracted_dir(pid)
        self.rs = P.resolved_dir(pid)
        ...
        # canonical curves live at canonical/curves.json in the production layout;
        # the older snapshot name resolved/canonical_curves.json is accepted so a
        # frozen fixture corpus can flow through the same code
        self.curves = ((self._j(P.canonical_dir(pid) / "curves.json", {}) or
                        self._j(self.rs / "canonical_curves.json", {}) or {})
                       .get("curves", []))
    ...
    def _sample_table(self):
        ...
        pdfs = self.pdfs()
        rows, header = ({}, "no source PDF for this paper")
        if pdfs:
    ...
    def pdfs(self):
        """The paper's source PDF(s): paper.pdf in the production layout, any
        PDFs under source/ in a snapshot corpus."""
        own = self.root / "paper.pdf"
        if own.exists():
            return [own]
        return sorted((self.root / "source").glob("*.pdf"))
...
# ---------------------------------------------------------------- production driver
def invariants(pid, o):
    """The invariants the semantic layer claims, computed rather than asserted."""
    ...
    # a canonical curve with no points has no data to lose: it is a recorded
    # extraction gap, counted separately, never a semantic-layer loss
    old_curves = {c["curve_id"] for c in src.curves if _npts(c)}
    empty_curves = sorted(c["curve_id"] for c in src.curves if not _npts(c))
    old_points = sum(_npts(c) for c in src.curves)
    ...
    return {
        "source_curves_preserved": {"canonical": len(old_curves), "semantic": len(rs_curves),
                                    "missing": sorted(old_curves - rs_curves),
                                    "empty_canonical_curves": empty_curves},
        "points_preserved": {"canonical": old_points,
                             "semantic": sum(r["n_points"] for r in o["result_series"])},
        "sweep_cases_carry_their_value": {"n": len(sweep_cases),
                                          "with_value": len(sweep_valued)},
        ...
        "data_source": dict(_Counter(r["data_source"] for r in o["result_series"])),
...
def invariants_ok(inv):
    """The hard subset that must hold for every paper."""
    return (not inv["source_curves_preserved"]["missing"]
            and inv["points_preserved"]["semantic"] >= inv["points_preserved"]["canonical"]
            and inv["simulation_never_a_case"]["simulation_runs_marked_as_case"] == 0
            and inv["samples_only_with_evidence"]["without_evidence"] == 0
            and inv["every_merge_has_evidence"]["without_evidence"] == 0)


def main(argv=None):
    import argparse
    ap = argparse.ArgumentParser(description="build the semantic layer")
    ap.add_argument("--paper", help="one paper id / DOI")
    ap.add_argument("--all", action="store_true",
                    help="every paper in the corpus manifest")
    ap.add_argument("--corpus-root", help="alternate corpus root (default: papers/)")
    args = ap.parse_args(argv)
    if args.corpus_root:
        P.set_corpus_root(args.corpus_root)
    if bool(args.paper) == bool(args.all):
        ap.error("exactly one of --paper / --all")
    pids = [P.paper_id(args.paper)] if args.paper else P.corpus_papers()
    allinv, failed = {}, []
    for pid in pids:
        o = build(pid)
        inv = invariants(pid, o)
        allinv[pid] = inv
        ok = invariants_ok(inv)
        if not ok:
            failed.append(pid)
        print("%-38s cases %-4d meas %-4d series %-4d rep %-4d points %-6d %s"
              % (pid, len(o["experimental_cases"]), len(o["measurements"]),
                 len(o["result_series"]), len(o["representations"]),
                 sum(r["n_points"] for r in o["result_series"]),
                 "ok" if ok else "INVARIANT FAILED"))
    if args.all:
        out = P.PAPERS / "_corpus" / "semantic_invariants.json"
        out.parent.mkdir(parents=True, exist_ok=True)
        out.write_text(json.dumps(allinv, indent=1, ensure_ascii=False))
        print("wrote %s" % out)
    if failed:
        print("INVARIANT FAILURES: %s" % ", ".join(failed))
        return 1
    return 0
