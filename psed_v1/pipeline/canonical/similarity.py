...
_HERE = os.path.dirname(os.path.abspath(__file__))          # …/pipeline/canonical
...
# =============================================================================
import re
ONTO = json.load(open(P.ONTOLOGY_JSON))
...
def _canonical_index():
    ...
    if _CANON_INDEX is None:
        idx = {}
        import glob
        for f in sorted(glob.glob(os.path.join(_REPO, "papers",
