...
These five were present in the legacy pipeline and absent from the current KB.
...
spec = u.spec_from_file_location("kb6", _PSED_ROOT / "pipeline" / "resolve" / "to_kb.py")
...
E = json.loads(P.resolved_json("10.1063_1.5028178", "experiments").read_text())
...
def conds(doi):
    ...
    p = P.resolved_json(doi, "experiments")
    ep = P.resolved_json(doi, "entities")
    ...
    gp = P.PAPERS / doi / "extracted" / "geometry.json"
    if gp.is_file():
        try:
...
g = json.loads((P.PAPERS / "10.1039_d0cp03358h" / "extracted" / "geometry.json").read_text())
