...
from pipeline.canonical.schema import REPO                    # noqa: E402
from pipeline.canonical import axis_roles as AR               # noqa: E402
from pipeline.canonical import granularity as G               # noqa: E402
...
def results(paper):
    f = P.resolved_json(paper, "results")
...
class CorpusInvariants(unittest.TestCase):
    ...
    def papers(self):
        return sorted(p.name for p in PAPERS.iterdir()
                      if P.resolved_json(p.name, "results").exists())
    ...
    def test_summary_counts_are_auditable(self):
        for p in self.papers():
            s = json.loads((P.resolved_json(p, "results")).read_text())["summary"]
