...
from pipeline.canonical import validate as V
from pipeline.canonical import sources as S
from pipeline.canonical.schema import REPO, Status
...
class TestRawImmutability(unittest.TestCase):
    ...
    def test_figure_data_json_is_never_written_by_the_canonical_layer(self):
        import pipeline.canonical.build_canonical as B
        import importlib.util as _iu
        A = _project.load('canonical_audit')
        for mod in (B, A, S):
...
class TestRecoveryMerge(unittest.TestCase):
    ...
    def test_36_recovery_files_carry_no_points(self):
        found = 0
        for p in sorted(P.glob_extracted("recovery/figure_semantics_v1.json")):
            found += 1
            doc = json.loads(p.read_text())
    ...
    def test_36b_point_counts_are_unchanged_by_recovery(self):
        ...
        for p in sorted(OUTPUT.glob("*/canonical/curves.json")):
            doc = json.loads(p.read_text())
            fd = json.loads((P.PAPERS / doc["doi"] / "extracted" / "figure_data.json").read_text())
            for c in doc["curves"]:
...
class TestDeterminism(unittest.TestCase):
    ...
    def test_37c_series_ids_are_deterministic(self):
        from pipeline.canonical import live
