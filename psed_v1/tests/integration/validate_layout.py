...
def main():
    ...
    for f in folders:
        ...
        if not P.extracted_dir(f).exists():
    ...
    for p in folders:
        fd = P.extracted_dir(p) / "figure_data.json"
        rf = P.resolved_json(p, "results")
        if fd.exists():
    ...
    for p in folders:
        xf = P.resolved_json(p, "experiments")
        if not xf.exists():
            continue
    ...
    missing = [p for p in folders
               if (P.resolved_dir(p)).exists()
               and not (P.review_path(p)).exists()]
    if missing:
