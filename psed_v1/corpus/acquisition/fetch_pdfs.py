...
def _email():
    """Resolved when the fetcher RUNS, not when it is imported.

    This used to `sys.exit()` at module scope, so merely importing the module --
    which the standalone test and any package scan do -- killed the process.
    """
...
def unpaywall(doi):
    try:
        r = _session().get(f"https://api.unpaywall.org/v2/{doi}", params={"email": EMAIL}, timeout=30)
...
def download(url, dest):
    try:
        r = _session().get(url, timeout=60, stream=True, allow_redirects=True)
    except requests.RequestException as e:
