...
# ─────────────────────────────────────────
# 종속변수 family (dep_overlap 게이트 완화용) — 현재 비활성
# ─────────────────────────────────────────
# 비워두면 dep_overlap 은 canonical 이름 '정확히' 일치하는 것만 겹침으로 본다.
# (normalized_thickness / film_thickness / thickness_per_cycle 를 서로 다른 것으로 취급)
# 다시 묶고 싶으면 아래에 매핑을 추가하면 된다. 예:
#   "film_thickness": "thickness", "normalized_thickness": "thickness", ...
DEP_FAMILY = {}
...
def load_experiments(output_dir: Path) -> list[dict]:
    ...
    for paper_dir in sorted(output_dir.iterdir()):
        ...
        for f in sorted(norm_dir.glob("experiment-*.json")):
            ...
            # 실제 데이터 시리즈 (viewer 에서 두 실험을 한 플롯에 겹쳐 그리는 용도)
